...
当前桌面应用清单版本为 `0.2.11`，用于未签名人工复测；它尚未打标签或发布，也不构成稳定版发布建议。
`0.2.10` 本地人工复测安装包及其来源证据，以及 `v0.2.9` 和更早版本的标签、Release、附件、安装包与来源证据继续保持不可变，不会被本轮候选覆盖。
`0.2.11` 必须先完成真实旧库覆盖升级和真实远程供应商人工复测，并等待单独发布指令；候选准备本身不等于发布完成。
所有已经发布的标签和附件均不得移动、删除或静默替换。
