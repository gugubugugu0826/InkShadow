...
test("Vite and release attestation share the audited entry, async and aggregate budgets", async () => {
  ...
  const viteEntryBudget = viteConfiguration.match(/ENTRY_CHUNK_BUDGET_BYTES\s*=\s*([^;]+);/u)?.[1];
  const attestationEntryBudget = releaseChecker.match(/entryChunk:\s*([^,\n]+),/u)?.[1];
  ...
  assert.ok(viteEntryBudget, "Vite entry chunk budget is missing");
  assert.ok(attestationEntryBudget, "release attestation entry chunk budget is missing");
  ...
  assert.equal(viteEntryBudget.replaceAll(/\s+/gu, ""), "352*1024");
  assert.equal(
    attestationEntryBudget.replaceAll(/\s+/gu, ""),
    viteEntryBudget.replaceAll(/\s+/gu, ""),
    "the build and release attestation must reject the same oversized entry chunk",
  );
