...
-- 0040 used ON DELETE SET NULL for the immutable rerun chain. Deleting an earlier snapshot
-- therefore attempted an UPDATE that was rejected by both the immutable trigger and the
-- rerun CHECK constraint. Rebuild the table without changing the released 0040 migration.
ALTER TABLE chapter_validation_snapshots
  RENAME TO chapter_validation_snapshots_0040_legacy;

CREATE TABLE chapter_validation_snapshots (
  ...
  supersedes_snapshot_id TEXT
    REFERENCES chapter_validation_snapshots(id) ON DELETE CASCADE,
...
SELECT
  ...
  result_json,
  generated_at
FROM chapter_validation_snapshots_0040_legacy
ORDER BY project_id, chapter_id, run_sequence;

-- The legacy self-FK still has SET NULL semantics, so make each rerun's temporary legacy
-- reference self-contained before clearing the copied rows. This happens in the migration
-- transaction and cannot leak partial state. It also avoids relying on PRAGMA foreign_keys=OFF,
-- which SQLite cannot change inside SQLx's per-migration transaction.
DROP TRIGGER chapter_validation_snapshot_binding_guard;
DROP TRIGGER chapter_validation_snapshot_version_guard;
DROP TRIGGER chapter_validation_snapshot_rerun_guard;
DROP TRIGGER chapter_validation_snapshot_immutable;

UPDATE chapter_validation_snapshots_0040_legacy
SET supersedes_snapshot_id = id
WHERE run_kind = 'rerun';

DELETE FROM chapter_validation_snapshots_0040_legacy;
DROP TABLE chapter_validation_snapshots_0040_legacy;

CREATE INDEX chapter_validation_snapshots_latest_idx
  ON chapter_validation_snapshots (project_id, chapter_id, run_sequence DESC);

CREATE INDEX chapter_validation_snapshots_version_idx
...
CREATE TRIGGER chapter_validation_snapshot_binding_guard
...
CREATE TRIGGER chapter_validation_snapshot_version_guard
BEFORE INSERT ON chapter_validation_snapshots
...
CREATE TRIGGER chapter_validation_snapshot_rerun_guard
BEFORE INSERT ON chapter_validation_snapshots
...
CREATE TRIGGER chapter_validation_snapshot_immutable
