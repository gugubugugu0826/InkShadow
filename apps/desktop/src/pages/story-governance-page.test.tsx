...
describe("StoryGovernancePage", () => {
  it("persists a human-confirmed formal record and governed memory", async () => {
    ...
    await user.click(screen.getByRole("tab", { name: "记忆治理" }));
  ...
  it("keeps What-if effects in a sandbox and promotes only an outline draft", async () => {
    ...
    await user.click(screen.getByRole("tab", { name: "What-if 沙盒" }));
    const createButton = screen.getAllByRole("button", { name: "新建 What-if" })[0];
  ...
  it("accepts a version-bound review item atomically into the formal record", async () => {
    ...
    await user.click(screen.getByRole("tab", { name: "审阅候选" }));
    const prepareButton = screen.getAllByRole("button", { name: "准备审阅候选" })[0];
    ...
    await user.click(screen.getByRole("button", { name: "保存为待审候选" }));
