...
> 更新日期：2026-08-22  
> 当前桌面应用工作树清单版本：`0.2.7`（尚未发布）  
> 最新公开工程预览版本：`v0.2.6`（预发布；标签与附件不得移动或静默替换）

本文是每次向 GitHub 上传源码或发布安装包时必须执行的检查清单。它只描述操作流程，
不替代 [`execution/RELEASE_CHECKLIST.md`](execution/RELEASE_CHECKLIST.md) 中的产品发布门禁。
`v0.2.3`–`v0.2.6` 的已发布事实记录在该门禁文档；后续版本在实际候选生成前仍保持“未运行”，
不得沿用任何历史版本数据。

## v0.2.7 待发布门禁

当前 `0.2.7` 只存在于工作树；完整源码门禁已通过，但尚未绑定干净唯一提交：

- 当前工作树的完整发布检查、全工作区测试、正式构建和原生层检查：通过；仍须由干净候选链重新绑定同一提交；
- 发布版浏览器端到端：未运行；
- 干净唯一提交、来源指纹和网页制品指纹：待生成；
- 未签名安装程序、发布清单和校验文件的文件名、字节与 SHA-256：待生成；
- 推送 `main`、创建 `v0.2.7` 带说明标签、公开工程预发布和全新目录回下载：未执行；
- 真实模型与 Windows 系统百分之二百缩放：未验证。

不得复用 `v0.2.6` 的提交、哈希、标签对象、持续集成编号或附件作为 `v0.2.7` 证据。
...
6. 不复用或强制移动已经发布的版本标签。`v0.2.0`、`v0.2.1`、`v0.2.2`、`v0.2.3`、`v0.2.4`、
   `v0.2.5` 和 `v0.2.6` 保持不可变；后续修复使用新的补丁版本。
...
版本号使用不带 `v` 的语义版本。当前工作树应用清单为尚未发布的 `0.2.7`；最新公开 `v0.2.6` 标签固定解析到
`b744d042eeafdd9db586388d71e701b1d937f366`，标签对象为
`f17fde56d83688bf6044a47c77794ebf0a46a936`。已发布的 `v0.2.5` 标签仍固定指向来源提交
`5b3e212cafde10cd75fa87b7b74bfdfff9347a3d`。`0.2.0` / `v0.2.0`、
`0.2.1` / `v0.2.1`、`0.2.2` / `v0.2.2`、`0.2.3` / `v0.2.3`、`0.2.4` / `v0.2.4` 与
`0.2.5` / `v0.2.5`、`0.2.6` / `v0.2.6` 都是既有公开版本，不能删除、移动或复用；新的
二进制修复必须继续使用新的补丁版本。
...
桌面应用 `v0.2.7` 的未来发行资产，也不要求随桌面标签同步。文档提到“当前应用版本”时必须明确
