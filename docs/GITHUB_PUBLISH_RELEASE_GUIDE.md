...
> 更新日期：2026-08-24  
> 当前桌面应用清单版本：`0.2.9`（本轮发布目标，尚不能预先视为已发布）  
...
本轮目标是在全部当前门禁通过后，从同一个干净提交准备 `v0.2.9` 标签、安装包和 Release。
在标签和 Release 实际创建并完成回下载复核前，本文不把 `v0.2.9` 记为已发布；最终状态以
[GitHub Releases](https://github.com/gugubugugu0826/InkShadow/releases) 为准。

## v0.2.8 本地人工复测包保留边界

0.2.8 人工复测包及其来源清单继续只保留在 `D:/InkShadow/installer/v0.2.8-manual-retest/`，不得
覆盖、移动或改作 `v0.2.9` 附件，也不得替换 `v0.2.7` 的任何公开附件。
...
版本号使用不带 `v` 的语义版本。当前桌面应用清单为 `0.2.9`，目标标签为 `v0.2.9`；只有完成
当前门禁、推送、标签创建、Release 上传和回下载复核后，才能把它记为已发布。截至候选准备时，
最新不可变公开工程预发行仍为 `v0.2.7`；该标签固定解析到
`cb97876894d6f02c4c901745c95533da7b0260fe`，标签对象为
