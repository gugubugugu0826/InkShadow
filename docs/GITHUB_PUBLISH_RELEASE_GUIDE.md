...
> 更新日期：2026-08-24  
> 当前桌面应用清单版本：`0.2.8`（仅本地人工复测包，未发布）  
> 最新公开工程预览版本：`v0.2.7`（未签名预发行；标签与附件不得移动或静默替换）

本文是每次向 GitHub 上传源码或发布安装包时必须执行的检查清单。它只描述操作流程，
不替代 [`execution/RELEASE_CHECKLIST.md`](execution/RELEASE_CHECKLIST.md) 中的产品发布门禁。
`v0.2.3`–`v0.2.7` 的已发布事实记录在该门禁文档；后续版本在实际候选生成前仍保持“未运行”，
不得沿用任何历史版本数据。

## v0.2.8 本地人工复测边界

0.2.8 当前只保留在 `D:/InkShadow/installer/v0.2.8-manual-retest/`，没有上传 GitHub。任何推送、标签或 Release 操作都必须等待单独发布指令；不得用该本地安装包替换 `v0.2.7` 的任何附件。正式发布前仍须重新运行完整候选链，并使用新的 0.2.8 标签和附件。
...
6. 不复用或强制移动已经发布的版本标签。`v0.2.0`、`v0.2.1`、`v0.2.2`、`v0.2.3`、`v0.2.4`、
   `v0.2.5`、`v0.2.6` 和 `v0.2.7` 保持不可变；后续修复使用新的补丁版本。
...
版本号使用不带 `v` 的语义版本。当前桌面应用清单 `0.2.7` 与已公开工程预发行一致；`v0.2.7`
标签固定解析到 `cb97876894d6f02c4c901745c95533da7b0260fe`，标签对象为
`37a40ddff9ea9aba27549f13f27718e319e2748e`。已发布的 `v0.2.6` 标签仍固定解析到
...
`0.2.5` / `v0.2.5`、`0.2.6` / `v0.2.6`、`0.2.7` / `v0.2.7` 都是既有公开版本，不能删除、移动或复用；新的
二进制修复必须继续使用新的补丁版本。
...
桌面应用 `v0.2.7` 的公开发行附件，也不要求随桌面标签同步。文档提到“当前应用版本”时必须明确
