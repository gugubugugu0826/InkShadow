...
6. 不复用或强制移动已经发布的版本标签。`v0.2.0` 和 `v0.2.1` 保持不可变；当前修复使用
   新补丁版本 `v0.2.2`。
...
版本号使用不带 `v` 的 SemVer，例如 `0.2.2`；Git 标签使用带 `v` 的形式，例如
`v0.2.2`。`0.2.0` / `v0.2.0` 与 `0.2.1` / `v0.2.1` 已是历史发布，当前补丁必须使用
`0.2.2` / `v0.2.2`，不得覆盖或移动任何已公开标签。
...
Desktop `v0.2.2` Release 资产，也不要求随桌面标签同步。文档提到“当前应用版本”时必须明确
...
这里要区分“仓库文件”和“Release 资产”：本地 `installer/` 文件夹永远不提交到源码仓库；
只有完成本节核验的单个最终安装包，才可以作为二进制资产单独上传到对应 GitHub Release。
...
```powershell
gh auth status
gh release create vX.Y.Z $installer.FullName `
...
- 从 GitHub 下载一次安装包，并对下载文件重新计算 SHA-256。
