...
> 更新日期：2026-08-25  
> 当前桌面应用清单版本：`0.2.12`；截至本轮候选形成阶段尚未发布，后续发布结果以 GitHub Release 和最终验证报告为准  
...
截至本轮候选形成阶段，`0.2.12` 尚未发布。全部必需门禁通过后，只能从与远端 `main` 一致的唯一干净提交生成
`InkShadow_0.2.12_x64-setup.exe`、来源清单、校验文件、完整中文验证报告和正式中文发布说明。
安装包未签名时必须明确标为“未签名工程预览”，GitHub Release 必须设为预发行。
...
0.2.11 人工复测安装包、来源清单、校验文件和验证报告，以及更早版本的标签、Release、附件、
安装包和来源证据均保持不可变。不得覆盖、移动或改作 0.2.12 发布附件，也不得替换任何既有公开附件。
...
版本号使用不带 `v` 的语义版本。当前桌面应用清单为 `0.2.12`；截至本轮候选形成阶段尚未发布。后续只有全部必需门禁通过、
来源提交已推送且工作区干净时，才能创建带说明标签和未签名预发行；实际结果以对应 GitHub Release 与最终验证报告为准。现有公开工程预发行记录以
...
完整验证报告和正式发布说明作为附件时，两者也必须上传。本轮 0.2.12 应恰有五个约定附件，
五者发布后都不得静默替换。
本轮五个附件分别是：`InkShadow_0.2.12_x64-setup.exe`、`inkshadow-release-manifest.json`、`SHA256SUMS`、最终验证报告和正式发布说明。`SHA256SUMS` 固定为四行，只覆盖其余四个附件，即安装程序、来源清单、最终验证报告和正式发布说明；它不能包含自身，否则会形成循环哈希。`SHA256SUMS` 自身的文件大小和 SHA-256 必须另行记录在 Release 页面与最终报告中，并在回下载后单独比对。
...
- 标签指向预期提交；
- 公开附件与本轮约定一致，0.2.12 的五个附件名称、大小和下载状态正确；
...
- [ ] Release 文案没有夸大未完成能力
- [ ] 未签名版本标为 Pre-release
- [ ] GitHub 附件数量和名称符合本轮约定；0.2.12 恰好五个附件
