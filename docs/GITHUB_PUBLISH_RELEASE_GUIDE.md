...
> 更新日期：2026-08-26  
> 当前桌面应用清单版本：`0.2.13`；只允许形成人工复测候选，取得真实安装复测和单独发布指令前不得创建标签或 Release  
...
全部必需门禁通过后，只能从唯一干净提交生成 `InkShadow_0.2.13_x64-setup.exe`、来源清单、校验文件、完整中文验证报告和正式中文候选说明。安装包未签名时必须明确标为“未签名工程预览”。本轮不创建标签或 GitHub Release；真实安装人工复测完成后仍需单独发布指令。
...
验证报告与更早版本的发布、候选和来源证据均保持不可变。不得覆盖、移动或改作 0.2.13 候选附件，也不得替换任何既有公开附件。
...
版本号使用不带 `v` 的语义版本。当前桌面应用清单为 `0.2.13`，本轮只允许形成人工复测候选。即使全部必需门禁通过，也必须等待 0.2.13 真实安装人工复测结果和用户单独发布指令，才能另行讨论标签或未签名预发行。最新公开版本 `v0.2.12` 固定解析到来源提交
...
完整验证报告和正式发布说明作为附件时，两者也必须上传。本轮 0.2.13 先在本地保留五个候选附件，不上传 GitHub：
`InkShadow_0.2.13_x64-setup.exe`、`inkshadow-release-manifest.json`、`SHA256SUMS`、完整中文验证报告和正式中文候选说明。`SHA256SUMS` 固定为四行，只覆盖其余四个附件；它不能包含自身，否则会形成循环哈希。`SHA256SUMS` 自身的文件大小和 SHA-256 必须另行记录。未来若取得真实安装复测和单独发布指令，仍须重新核对该次获授权发布的附件范围，不得把候选说明自动当作发布说明。
...
- [ ] Release 文案没有夸大未完成能力
- [ ] 未签名版本标为 Pre-release
- [ ] GitHub 附件数量和名称符合该次单独发布指令；0.2.13 候选阶段不得勾选
