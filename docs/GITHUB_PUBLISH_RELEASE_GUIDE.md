...
> 更新日期：2026-08-25  
> 当前桌面应用清单版本：`0.2.11`（未签名人工复测候选；未打标签、未发布）  
...
本轮只在全部本地门禁通过后，从同一个干净提交生成并保留 `InkShadow_0.2.11_x64-setup.exe`、
来源清单、校验文件和完整中文验证报告。安装包必须明确标为“未签名人工复测候选”。本轮不授权创建
`v0.2.11` 标签或 GitHub Release；真实旧库覆盖升级和真实远程供应商人工复测完成后仍须等待单独发布指令。
...
0.2.10 人工复测安装包、来源清单、校验文件和验证报告，以及更早版本的标签、Release、附件、
安装包和来源证据均保持不可变。不得覆盖、移动或改作 0.2.11 候选附件，也不得替换任何既有公开附件。
...
版本号使用不带 `v` 的语义版本。当前桌面应用清单为 `0.2.11`，只用于未签名人工复测候选；
本轮没有标签或 Release 目标，不能因完成本地打包而自行创建。现有公开工程预发行记录以
