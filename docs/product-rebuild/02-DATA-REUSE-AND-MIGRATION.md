...
- 当前工作树只向前新增 `0079_story_fact_evidence.sql` 和 `0080_candidate_selection_action.sql`，桌面原生连续序列对应 Tauri `82`–`83`；不得修改 `0078` 或任何更早迁移的名称、SQL 字节、顺序和校验值。
...
> 规则：只向前追加；当前 0.2.14 发布目标的工作树前向上限为 Data `0080` / Tauri `83`；0.2.13 候选与 v0.2.12 及更早迁移、标签、附件和来源证据保持不可变。
