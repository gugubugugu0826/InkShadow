...
> 当前迁移上限：Data `0064` / Tauri `67`  
> 规则：只向前追加；当前应用版本为 `0.2.2`。

## 1. 迁移原则

1. 只追加新迁移，不修改当前已登记的 Data `0001`–`0059` 和 story-core `0001`–`0003`。
...
当前共有 143 张应用表：恢复权威白名单为 142 张表，另有 1 张不恢复的临时租约表。`project_remote_dispatch_leases` 是单独的短期网络事实，不进入
