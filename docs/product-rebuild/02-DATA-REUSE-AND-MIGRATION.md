...
- `0075` 聚焦迁移、维护和数据仓库为 3 个文件、75 项通过，当前工作树统一门禁也已通过；真实旧库安装程序升级和完整备份恢复仍待验证。

> 文档状态：`AUTHORITATIVE_CURRENT`  
> 当前工作树迁移上限：Data `0075` / Tauri `78`  
> 规则：只向前追加；当前应用清单为 `0.2.7`，最新已发布版本为 `v0.2.6` 工程预览版。

`0072`–`0075` / Tauri `75`–`78` 属于尚未发布的 `v0.2.7` 工作树，应用清单已升至 `0.2.7`。
当前工作树最终全量已经通过；真实旧库升级、Windows 安装程序备份恢复、干净提交、推送、打包和发布仍待验证；
