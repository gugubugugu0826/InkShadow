...
> 当前工作树迁移上限：Data `0071` / Tauri `74`  
> 规则：只向前追加；当前应用清单为 `0.2.6`，最新已发布版本为 `v0.2.5` 工程预览版。

`0071` / Tauri `74` 属于尚未发布的 `v0.2.6` 修复工作树，应用清单已升至 `0.2.6`。
最终全量、旧库升级、真实 Windows Tauri 备份恢复、干净提交、推送、打包和发布仍待验证；
`v0.2.5` 的来源提交和冻结迁移上限保持历史事实。

## 1. 迁移原则

1. 只追加新迁移，不修改已发布 `v0.2.5` 的 Data `0001`–`0070` 和 story-core `0001`–`0003`。
