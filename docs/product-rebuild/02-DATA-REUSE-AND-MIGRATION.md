...
## 2026-08-30 0.2.16 当前数据边界

- 本轮没有新增、修改或重排数据库迁移；Data 前向上限保持 `0082`，桌面原生连续序列保持 Tauri `85`。`0082` 及全部更早迁移的名称、SQL 字节、顺序和校验值保持不变。
- F17 不通过放宽 `0080_candidate_selection_action.sql` 解决。生产隔离结果提交必须把改写、润色、扩写或缩写的准确 `selection_action` 同时接入查询、写入和幂等比较；历史空值仍按已发布兼容合同读取，新记录继续受不可变守卫保护。
- 章节隐私继续使用章节权威行和预期隐私修订；示例与测试身份继续复用项目显示身份；通知清除只把已读通知转为隐藏状态。上述操作不得改写正文、不可变版本、灵感、设定、历史候选或任务审计。
- 0.2.16 发布门禁必须覆盖 v0.2.3 至当前连续升级、完整用户数据备份与恢复、真实临时 SQLite 关闭重开，以及正文、版本、灵感、设定、历史候选和通知审计摘要守恒。
- 报告引用的现场数据库和截图目录未取得，不能把合成升级、临时 SQLite 或内存存储测试写成现场旧库复现。最终数据测试统计必须绑定唯一发布提交后再填写。

## 2026-08-29 0.2.15 历史数据边界

- 本轮没有新增、修改或重排数据库迁移；Data 前向上限保持 `0082`，桌面原生连续序列保持 Tauri `85`。
- 写作技能与个人技能复用既有不可变技能定义、版本和项目绑定表，不建立第二事实源；正文、章节版本、灵感、设定、候选、恢复记录和历史调用的权威关系不变。
- 0.2.15 发布门禁必须覆盖 v0.2.3 至当前连续升级、完整用户数据备份与恢复、真实临时 SQLite 关闭重开，以及正文、版本、灵感、设定和历史候选摘要守恒。
- `0082` 及所有更早迁移的名称、SQL 字节、顺序和校验值保持不变。现场用户数据库未取得，不能把合成升级测试写成现场旧库验证。

## 2026-08-27 0.2.14 当前向前迁移

- 当前工作树在既有迁移之后继续只向前新增 `0082_author_recovery_records.sql`，桌面原生连续序列对应 Tauri `85`；不得修改 `0081` 或任何更早迁移的名称、SQL 字节、顺序和校验值。
- `0079` 为同一受治理故事事实追加多处不可变正文依据。原始来源列继续保留，后续证据使用独立关系追加；作者已经确认、修改或放弃的决定不因新版本再次整理而被覆盖。
- `0079` 新表纳入维护、备份和恢复白名单，并以外键、UTF-16 范围核对和不可变触发器拒绝跨项目、跨章节、无效字节边界或后续改写的证据。缺少该表的旧备份在恢复时按既有事实来源事务性回填，不删除原事实。
- `0080` 为新选区隔离结果保存改写、润色、扩写或缩写的准确动作。历史空值保持为空并走明确兼容分支，新记录必须保存四选一动作，已经保存的动作不可修改。
- `0081` 通过复用已读取的 UTF-8 前导字节、原生计算 UTF-16 总长并把递归限制到证据末端，保持 `0079` 的项目归属、总长、边界和精确摘录核对；约四万字三处证据聚焦用例由修复前约 2.43 秒降至约 0.75 秒，固定 2 秒门禁未放宽。
- 故事核心 148 项、数据层 481 项和 Rust 207 项已通过，覆盖新库、v0.2.3 至当前连续升级、真实临时 SQLite 关闭重开、旧备份回填和当前备份恢复。现场数据库未取得；正式构建与完整候选链仍未通过，不能据此宣称 0.2.14 已发布。

## 2026-08-26 0.2.13 当前边界

- 本轮没有新增、修改或重排数据库迁移；Data 前向上限保持 `0078`，Tauri 保持 `81`。
- v0.2.12 已冻结 `0078` / `81`。0.2.13 的历史版本恢复、设定幂等和项目包修复不删除坏行、不清库，也不改变正文与不可变版本表结构。

## 2026-08-25 0.2.12 当前向前迁移

- 当前工作树只新增 `0078_generation_attempt_prose_invocation.sql`，桌面原生连续序列对应 Tauri `81`；0.2.12 尚未打标签或发布。
- `0078` 不新增字段或表，只重建 `0075` 建立的不可变生成尝试隐私守卫，使 `continuation` 与 `prose_generation` 都能保存精确 Model Hub 调用标识，并继续要求隐私策略和数据去向与调用事实一致。
- `0075_generation_attempt_privacy_snapshot.sql` 的内容和校验未修改；所有已发布迁移、顺序和校验值保持不变。

## 2026-08-24 v0.2.9 冻结迁移

- `0076_direct_local_story_fact_author_revision.sql` 为直接模式本地整理出的事实增加带审计的作者修订，不改变权威正文和不可变版本。
- `0077_project_display_identities.sql` 增加不含作品内容的项目显示标识与修订历史，用于界面诊断和可辨识导航，不替代项目 UUID。
- 桌面原生连续序列对应前进到 Tauri `79`–`80`；所有较早迁移及校验值保持不可变。
- 两项已随 `v0.2.9` 来源提交 `54d9647031bb97b4fc9f021d3b1acca7f6d25c47` 冻结。0.2.10 与 0.2.11 均没有新增迁移；当时的前向上限为 Data `0077` / Tauri `80`。

## 历史：2026-08-22 v0.2.7 追加迁移

- `0072_ai_candidate_purpose.sql` 为隔离结果增加用途，明确区分正文结果与续写方向，避免方向被当作正文接受。
- 0073_story_fact_user_revisions.sql 只为 structuredValue = null 的纯文本用户事实补充修改、删除、历史恢复和重复事实合并记录；结构化或因果事实不走这套通用重写/合并链。
- `0074_chapter_version_story_fact_responsibility.sql` 在不可变章节版本上保存本地故事资料整理责任；旧版本默认关闭，责任不可修改，后台恢复不读取当前界面模式猜测。
- `0075_generation_attempt_privacy_snapshot.sql` 为生成尝试保存隐私快照版本、隐私策略、数据去向和同一次模型调用标识；迁移前旧行保留空值，新行必须完整、一致且不可修改。
- 结构化或因果事实当前只允许不破坏结构的证据查看、固定、删除和删除恢复；需要改变结构时必须进入对应的结构化事务，不能把普通文本历史冒充为合法结构。
- 桌面原生迁移序号随四项追加迁移前进到 `78`；任何已发布迁移和校验值均未修改。
- 维护与备份恢复必须覆盖新增用户数据；方向、摘要、索引等可重建资料不得成为正文恢复依赖。
- `0075` 聚焦迁移、维护和数据仓库为 3 个文件、75 项通过，`v0.2.7` 发布候选统一门禁也已通过；真实旧库安装程序升级和完整备份恢复仍待验证。

> 文档状态：`AUTHORITATIVE_CURRENT`  
> `v0.2.7` 发布迁移上限：Data `0075` / Tauri `78`  
> 规则：只向前追加；当前工作树前向上限为 Data `0082` / Tauri `85`；已发布迁移、标签、附件和来源证据保持不可变。

`author_recovery_records` 按“项目＋恢复种类”保存作者尚未完成的本地工作流快照。写入和删除均以修订号比较交换，坏记录或未来 schema 保留原始 JSON，不会被自动删除或覆盖；该表进入 SQLite 的 VACUUM 备份和事务恢复，旧备份允许缺少此表。

`0072`–`0075` / Tauri `75`–`78` 已随 `v0.2.7` 最终候选冻结公开。
最终候选、远端门禁、未签名打包和公开预发行已经完成；真实旧库升级与 Windows 安装程序备份恢复仍未执行；
`v0.2.6` 的来源提交和 Data `0071` / Tauri `74` 冻结迁移上限保持历史事实。

## 1. 迁移原则

1. 只追加新迁移，不修改已发布 `v0.2.6` 的 Data `0001`–`0071` 和 story-core `0001`–`0003`。
...
`0071_model_capability_probe_invocation_ledger.sql` / Tauri `74` 以前向表重建为模型调用记录新增
独立 `capability_probe` 任务，并在迁移前后校验既有调用行数守恒。`model_capability_scans` 追加
可空、唯一的调用外键；插入约束要求扫描与同一目录项、状态匹配的终态能力验证调用记录关联，
更新约束禁止事后改绑。它不保存能力验证短句、作品正文、完整模型地址或密钥。

备份恢复继续覆盖相同 172 张权威表。Tauri `74` 来源会精确恢复能力扫描外键；从 Tauri `73`
旧备份恢复时，维护服务先检查旧扫描表列，缺少外键列则显式插入 `NULL`，保留旧能力证据以及
正文、不可变版本、AI 建议草稿、任务和历史调用。恢复后仍必须通过完整性、外键与结构核验。
