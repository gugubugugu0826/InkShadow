...
> 当前迁移上限：Data `0055` / Tauri `58`  
> 规则：只向前追加；应用版本仍为 `0.2.0`。

## 1. 迁移原则

1. 只追加新迁移，不修改当前已登记的 Data `0001`–`0055` 和 story-core `0001`–`0003`。
...
迁移 `0052`–`0055` 继续采用只向前追加：`0052` 保存连续故事状态的精确路由回执，`0053`
...
当前共有 142 张应用表：恢复权威白名单为 141 张表，另有 1 张不恢复的临时租约表。`project_remote_dispatch_leases` 是单独的短期网络事实，不进入
