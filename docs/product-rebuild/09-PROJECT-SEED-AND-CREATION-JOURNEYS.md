...
实现，不是当前未发布工作树事实。当前修复先持久化三个隔离开头槽，并等待作者明确选择一个可用
...
ProjectSeed，再确认恢复得到原 payload 和 revision；截至 Data `0065` / Tauri `68`，当前共有
167 张应用表，恢复权威白名单为 166 张表。1 张短期远程派发租约表不恢复，4 个可重建派生根表在同一恢复事务中清空。
