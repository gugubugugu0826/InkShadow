...
直接模式只显示返回、“一句话就够了”、一句话输入、开始创作和直接写空白正文。开始创作先保存作品与用户想法，再按真实可用配置请求一个开头；失败时保留作品和输入，允许重试或直接手写，不生成三份假方案。单开头计划和持久化已进入工作树，但“生成后自动接受并直接进入正文”仍待用户对新授权策略作独立明确授权，当前不得写成已经完成。专业模式继续保留多方案和人工决定。

> 文档状态：`SUPPORTING_CURRENT`  
> 当前应用清单：`0.2.10`（未签名人工复测候选；未打标签、未发布）；最新公开版本：[`v0.2.9`](https://github.com/gugubugugu0826/InkShadow/releases/tag/v0.2.9)（未签名工程预发行），发布来源提交 `54d9647031bb97b4fc9f021d3b1acca7f6d25c47`；设计基线：`DESIGN v0.3.1b`
...
实现，不是 `v0.2.5` 冻结源码事实。`v0.2.5` 历史实现先持久化三个隔离开头槽，并等待作者明确选择一个可用
...
`v0.2.6` 发布前工作树曾进一步修复批次提前结束，相关实现后来随公开工程预发布冻结：三个稳定槽现在各自等待并结算，首个本地持久化
