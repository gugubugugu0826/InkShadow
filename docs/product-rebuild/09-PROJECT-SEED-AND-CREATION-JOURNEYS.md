...
一句话开书使用有明确终点的动态问题计划：初始 5 个重点，只有回答暴露新的必要边界时才追加
尚未问过的重点，总量最多 12 个。快照保存唯一问题键、N/M、完成百分比、剩余重点和扩展原因；
回答或跳过会减少剩余重点，没有新重点时进入完成状态。作者始终可以返回、跳过或直接结束创建。
这不是“三问上限”，也不会无限追问。每次回答只重算并保存 ProjectSeed，不会自动重写开头；
全部问题完成后，只有作者明确选择时才执行一次基于全部回答的整合重写。
...
ProjectSeed，再确认恢复得到原 payload 和 revision；截至 Data `0059` / Tauri `62`，当前共有
143 张应用表，恢复权威白名单为 142 张表。1 张短期远程派发租约表不恢复，4 个可重建派生根表在同一恢复事务中清空。
