...
> 基线日期：2026-08-01  
> 最近复核：2026-08-26（DESIGN v0.3.1b 与 0.2.13 当前人工复测候选）  
> 文档状态：`AUTHORITATIVE_CURRENT`（本目录索引与冲突规则）  
> 应用清单版本：`0.2.13`（未签名人工复测候选；未打标签、未发布）；v0.2.12 及更早发布证据保持不可变；设计基线：`DESIGN v0.3.1b`  
...
当前迁移头为数据层 `0078_generation_attempt_prose_invocation.sql`；78 个数据迁移与 3 个故事核心迁移合并为原生层内部版本 `81`。
`0071` 已随 `v0.2.6` 冻结；`0072`–`0075` 保留正文用途、故事资料修订、本地整理责任和不可变隐私快照合同，`0076`–`0077` 已随 `v0.2.9` 冻结作者修订与项目显示身份。当前 0.2.12 的 `0078` 不新增字段或表，只扩展不可变生成尝试隐私守卫，使 `continuation` 和 `prose_generation` 均可保存精确 Model Hub 调用标识；没有修改 `0075` 或任何已发布迁移及校验。恢复清单仍覆盖全部用户创作表，唯一不恢复的应用表仍是临时远程派发租约；真实旧库安装升级和最终安装程序恢复仍待验证。
