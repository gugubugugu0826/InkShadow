...
> 基线日期：2026-08-01  
> 最近复核：2026-08-31（DESIGN v0.3.1b 与 0.2.16 当前发布目标）  
> 文档状态：`AUTHORITATIVE_CURRENT`（本目录索引与冲突规则）  
> 当前发布目标：`0.2.16`；选区结果、章节隐私、写作技能、检查入口、示例编辑提示、版本恢复和普通界面修复正在绑定当前自动化与唯一发布来源；现有 v0.2.15 及更早标签、Release、附件和发布证据保持不可变；设计基线：`DESIGN v0.3.1b`  
...
0.2.16 的当前产品增量坚持同一安全边界：四项选区结果保存准确动作并保持隔离，本地提交失败时由作者决定复制或放弃片段后安全离开；章节隐私成功与失败都有可见反馈，私密正文继续禁止远程发送；自定义技能只按明确任务范围参与续写等已经接入的资料编译，选区处理尚未接入自定义技能；示例编辑提示、恢复和检查补齐作者可理解的状态，任务通知可以跨全部批次标为已读或清除且不删除审计关系，既有示例身份和作品库分区保持不变。本轮不新增迁移，不修改任何已发布迁移或校验值。完整门禁、真实安装和人工验收的结论只能由本轮当前运行证据给出。
...
`0071` 已随 `v0.2.6` 冻结；`0072`–`0075` 保留正文用途、故事资料修订、本地整理责任和不可变隐私快照合同，`0076`–`0077` 已随 `v0.2.9` 冻结作者修订与项目显示身份，`0078` 已随 `v0.2.12` 冻结生成尝试的正文调用关联。`0079`–`0081` 保存多处正文证据、选区动作和长正文证据守卫性能，`0082` 保存进入备份恢复合同的作者恢复记录。0.2.16 不新增迁移，也不修改任何已发布迁移及校验。恢复清单仍覆盖全部用户创作表，唯一不恢复的应用表仍是临时远程派发租约；真实旧库安装升级和最终安装程序恢复仍待验证。
