...
> 基线日期：2026-08-01  
> 最近复核：2026-09-06（DESIGN v0.3.1b 与 0.2.18 修复发布）  
> 文档状态：`AUTHORITATIVE_CURRENT`（本目录索引与冲突规则）  
> 当前发布目标：`0.2.18`；本地门禁全部通过后授权安全推送与发布，未签名包仅预发行；既有标签、附件和 0.2.17 人工复测包保持不可变；设计基线：`DESIGN v0.3.1b`  
> 适用范围：桌面端个人创作主链路、Model Hub、后续小说智能内核

当前增量和延期边界见 [0.2.18 实现说明](V0218-CURRENT-REALITY.md)，缺陷与证据见 [0.2.18 执行记录](../execution/2026-09-06-V0218-REPAIR.md)。下文旧版本描述是历史能力基线，旧轮次的发布授权限制不适用于本轮；最终发布事实必须核对随安装包交付的验证报告。
