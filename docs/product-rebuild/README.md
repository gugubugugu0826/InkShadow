...
> 基线日期：2026-08-01  
> 最近复核：2026-08-23（DESIGN v0.3.1b、历史 v0.2.6 与已公开工程预发行 v0.2.7）  
...
当前迁移头为数据层 `0075`；75 个数据迁移与 3 个故事核心迁移合并为原生层内部版本 `78`。
`0071` 已随 `v0.2.6` 冻结；`0072` 区分正文与方向用途，`0073` 保存用户故事资料修订，`0074`
保存不可变版本的本地整理责任，`0075` 保存隐私快照和同一次调用标识。当前恢复白名单仍为 172 张表；
唯一不恢复的应用表仍是临时远程派发租约。当前工作树的升级、维护、备份恢复自动化及完整源码门禁
已经通过；真实旧库安装升级和最终安装程序恢复仍待验证。
