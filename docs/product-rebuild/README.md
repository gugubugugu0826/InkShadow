...
> 基线日期：2026-08-01  
> 最近复核：2026-08-27（DESIGN v0.3.1b 与 0.2.14 当前发布目标）  
> 文档状态：`AUTHORITATIVE_CURRENT`（本目录索引与冲突规则）  
> 当前发布目标：`0.2.14`；数据、桌面、全仓自动化、桌面正式构建与生产浏览器旅程已通过，干净来源候选链、真实技能、真实安装和人工验收尚未闭合；0.2.13 五项候选附件与 v0.2.12 及更早发布证据保持不可变；设计基线：`DESIGN v0.3.1b`  
...
0.2.14 的当前产品增量坚持四项边界：专业创建的故事资料先待作者确认，视角、文风和禁止项作为写作偏好或独立约束；同一事实跨任务和不可变版本收敛身份但保留所有独立证据；四项选区技能保存准确动作且结果继续隔离；生成中离开页面必须先说明影响并诚实结算发送状态。Data `0079`–`0080`／Tauri `82`–`83` 只向前追加。UI 42／42、全仓 3,945 项、Rust 207 项、桌面正式构建和生产 Chromium 26／26 已通过；正式前端为 7,337,824／7,340,032 字节，余量 2,208 字节。首次完整发布检查因误删 `.ink-prose` 合同而停止，修复后相关分项分别重跑通过，但没有取得修复后单次完整退出 0 的记录；正式发布端到端又按设计拒绝未提交工作区并在启动旅程前停止，完整候选链未运行。三份 0.2.13 测试结果原件未取得，真实模型、真实安装和真人交互未执行，不能把当前代码和自动化结果写成最终发布验证。
