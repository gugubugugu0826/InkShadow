...
> 基线日期：2026-08-01  
> 最近复核：2026-08-29（DESIGN v0.3.1b 与 0.2.15 当前发布目标）  
> 文档状态：`AUTHORITATIVE_CURRENT`（本目录索引与冲突规则）  
> 当前发布目标：`0.2.15`；开书、写作技能、章节安全、版本恢复、规划、导出和兼容性修复正在绑定当前自动化与唯一发布来源；现有 v0.2.14 及更早标签、Release、附件和发布证据保持不可变；设计基线：`DESIGN v0.3.1b`  
...
0.2.15 的当前产品增量坚持同一安全边界：开书方案有序调度且每个方案独立结算，跳过 AI 继续使用同一作品；启用的写作技能只有在真实进入资料编译链时才显示为采用；大幅删除先保留恢复草稿并等待作者确认；版本恢复创建新版本而不改写历史；原生导出明确等待、取消和成功状态。旧结构人物、连接重试和向量能力验证继续走现有生产存储与发送边界。本轮不新增迁移，不修改任何已发布迁移或校验值。完整门禁、真实安装和人工验收的结论只能由本轮当前运行证据给出。
