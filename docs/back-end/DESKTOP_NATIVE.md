...
> 当前交付目标：`0.2.18`；迁移上限仍为 Data `0082`／Tauri `85`，没有新增或修改已发布迁移；完整本地门禁通过后可安全发布，未签名包仅预发行；设计基线：`DESIGN v0.3.1b`  
> 覆盖范围：`apps/desktop/src-tauri`、本地 SQLite 原生桥、自动备份、系统凭据库、原生网络、项目密钥、安全更新与系统容量

## 0. 0.2.18 当前增量与边界

本轮保留原生事务、隐私发送回执和恢复安全边界。应用与安装程序版本统一为 0.2.18；必须直接检查产物版本、应用载荷架构和未签名状态。原生全量结果只能绑定最终干净提交；真实安装升级、系统凭据交互和现场数据库验证未执行。详见 [执行记录](../execution/2026-09-06-V0218-REPAIR.md)。以下 0.2.17 段落保留为历史说明，其发布授权限制已被本轮用户指令替代。

## 0.1. 0.2.17 人工复测候选历史基线

- 编辑器任务草稿、统一提示词策略和资料编译发生在桌面渲染层与共享包；原生层不解释作者要求、不补写技能，也不把结构化结果转换成正文。只有已经通过任务身份、资料、输出和隐私围栏的请求才能进入原生网关，返回内容仍必须先保存为隔离结果。
- 原生模型网关继续以调用标识和发送回执作为唯一发送事实。确认前取消不进入网关；进入发送边界后结果不确定时保留调用记录并由上层显示“结果需要核对”，自动重试保持为零，不能因页面离开、后台任务或租约恢复而再次发送。
- 项目或章节要求本地处理时，正文、规划、摘要、审稿、一致性、远程向量、后台整理和自动任务都必须在原生网络前停止。项目远程派发租约只含项目、修订、目的和时限等白名单元数据，不含正文、提示词或凭据；租约不能改变隐私权威，也不能授权新的网络调用。
- 本地事实整理和摘要重建可以在不经过模型网关的情况下继续；它们的失败是派生失败，不得回滚已接受正文。普通界面是否显示摘要已更新，依据不可变来源版本，不由原生层按字数或时间猜测。
- 0.2.17 没有新增原生命令或数据库迁移。当前迁移上限仍为 Data `0082_author_recovery_records.sql`／Tauri `85`，所有已发布迁移字节和校验值保持不变。
- 当前证据来自可控网关、临时 SQLite、Rust 和浏览器自动化；没有执行真实供应商请求、真实 Windows 凭据交互、0.2.17 安装程序人工验证、系统级百分之二百缩放或真人视觉。当前只允许形成五项本地人工复测附件，不创建标签、GitHub Release 或公开附件。
...
- 0.2.16 源码保持 Data `0082_author_recovery_records.sql`／Tauri `85`；v0.2.15 及更早迁移字节、校验和与发布对象保持不变。
- 该节自动化没有执行真实供应商请求、真实 Windows 凭据交互或 0.2.16 安装程序人工验证；临时 SQLite、可控 HTTP 和浏览器回归不得表述为真实安装结论。
