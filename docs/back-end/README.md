...
> 基于源码复核：2026-09-06  
> 当前交付目标：`0.2.18`；迁移上限仍为 Data `0082`／Tauri `85`，没有新增或修改已发布迁移；全部本地门禁通过后授权安全推送与发布，未签名包仅预发行  
> 设计基线（Design Baseline）：`DESIGN v0.3.1b`  
> 文档性质：当前实现指引，不把计划代码描述成已部署能力

## 0. 0.2.18 当前增量

隐私和硬规则写入增加权威回读；技能采用说明重放已有不可变配置，逐句设定继续复用恢复记录。没有新增字段、迁移或平行权威存储。最终验证、未执行项与产物来源见 [本轮执行记录](../execution/2026-09-06-V0218-REPAIR.md) 和随安装包交付的报告。旧轮次“仅本地候选”的限制不适用于已另行授权的 0.2.18。

## 0.1. 0.2.17 人工复测候选历史基线

- 编辑器的续写主入口与选区工具已经分离：续写入口常驻，选区工具只响应当前有效选区。任务要求草稿按项目、章节、正文版本、任务、选区和会话区分，迟到恢复不能覆盖当前选择；成功、失败、取消和离页都必须结清当前任务，隔离结果在作者接受前仍不得写入正文。
- 正文类任务共用统一提示词优先级和可见输出边界。安全、隐私、已保存正文与锁定事实高于任务、作者本次要求、规划、技能和偏好；输出只能是可直接审阅的小说正文，分析、解释、代码围栏、内部标签和结尾元说明会在进入隔离结果前失败关闭。结构化任务继续使用独立、带版本的契约。
- 普通界面的篇幅只表达短、中、长及任务相关的自然停止条件；高级目标字数明确允许约正负百分之二十浮动。输出上限、费用保护和部分输出结算仍由请求与账本层执行，不依赖提示词承诺固定字数。
- 资料编译先为当前任务、安全规则、最新正文和直接相关的锁定资料保留预算，再处理可选旧资料；相同资料按来源、版本、定位和内容指纹去重，并记录采用、省略、原因和预算。自定义技能只有经过适用任务匹配、实际编译、发送前采用快照和调用记录才能显示为本次采用。
- 事实整理只从明确语义与不可变版本证据形成待确认记录；章节摘要的新旧状态比较来源版本而不是字数，一致性调查区分已检查未发现、证据不足、本次未执行和检查失败。
- 私密章节在正文生成、规划、摘要、审稿、一致性、远程向量、后台整理和自动任务的真实发送边界前统一失败关闭；本地整理仍可执行并明确标为本地。远程派发租约只保存不含用户内容的并发事实，不能绕过项目隐私权威或触发自动重发。
- 当前源码仍以 Data `0082_author_recovery_records.sql`／Tauri `85` 为迁移上限；0.2.17 不新增迁移，不改动任何已发布迁移字节或校验值，也不建立平行事实源。
- 当前结论只绑定源码和当前自动化。真实付费供应商、0.2.17 安装程序、真实 Windows 凭据管理器、系统级百分之二百缩放和真人视觉均未执行；本轮只形成五项本地人工复测附件，不创建或暗示已授权 `v0.2.17` 标签、GitHub Release 或公开附件。
...
- 0.2.16 源码保持 Data `0082_author_recovery_records.sql`／Tauri `85` 上限；该版本不新增迁移，不改动 v0.2.15 及更早迁移字节、校验和、标签、Release 或附件。
- 该节结论来自 0.2.16 当时的源码和自动化；现场数据库、截图、完整诊断与供应商响应未取得，真实供应商调用和 0.2.16 真实安装程序人工验证当时尚未执行。
...
0.2.17 历史差异与边界见
[`../execution/2026-09-01-V0217-CANDIDATE.md`](../execution/2026-09-01-V0217-CANDIDATE.md)；0.2.16 的冻结发布事实见
...
当前交付目标为 0.2.18；提交、安装包和发布状态只能由本轮最终证据确认。已有标签、附件、安装包和来源证据保持不变。当前授权仅在全部本地门禁通过后安全推送并发布，不授权付费模型调用、强制推送或替换旧附件。
