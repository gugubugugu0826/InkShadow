...
> 当前交付目标：`0.2.18`；完整本地门禁通过后授权安全推送与发布，未签名包仅预发行；设计基线：`DESIGN v0.3.1b`  
> 覆盖范围：`packages/*/src` 的 17 个工作区包，以及 `packages/data/migrations` 的 82 个本地数据库迁移（当前源码前向上限为 `0082`）

## 0. 0.2.18 当前增量

应用用例与事实用例增加写后权威回读，拒绝仅有写入回执却未持久化的成功假象；共享弹窗完善详情开关焦点，字数显示保留技术校验；纯文本导出保留字内与转义下划线。没有新增数据库字段或迁移，所有历史数据仍由现有备份恢复合同覆盖。最终检查结果见 [本轮执行记录](../execution/2026-09-06-V0218-REPAIR.md) 和安装包随附报告。以下旧轮次授权只作历史说明。
