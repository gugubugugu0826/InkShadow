...
> 当前工作树应用清单版本：`0.2.7`；最新公开版本：`v0.2.6`（未签名工程预发布；不可移动或复用）  
> 设计基线（Design Baseline）：`DESIGN v0.3.1b`  
> 最近治理复核：2026-08-22  
> [`v0.2.6` 工程预发布](https://github.com/gugubugugu0826/InkShadow/releases/tag/v0.2.6)：来源提交 `b744d042eeafdd9db586388d71e701b1d937f366`；标签和公开附件保持不可变。当前 `v0.2.7` 工作树尚未形成候选或制品。  
> 注意：设计版本用于界面与交互验收，不会自动改变应用版本。

当前共享工作树正在准备 `v0.2.7` 直接模式与数据保护修复；应用清单已升至 `0.2.7`，
当前工作树完整源码门禁已经通过；真实模型、系统百分之二百缩放、干净候选、端到端、打包和发布仍待完成。当前事实和待验证边界见
[`execution/2026-08-22-V027-DIRECT-MODE-REMEDIATION.md`](execution/2026-08-22-V027-DIRECT-MODE-REMEDIATION.md)。
该工作树记录不会移动或重解释 `v0.2.6` 及更早历史标签、来源哈希和公开附件。
...
- `DESIGN/`：`TARGET_BASELINE`。DESIGN v0.3.1b 规定视觉与交互目标；当前工作树应用清单为
  0.2.7，最新公开工程预发布为 v0.2.6。其发布页恰有安装包、发布清单与
  `SHA256SUMS` 三个附件，精确大小和 SHA-256 见 v0.2.6 发布门禁。
