...
> 更新日期：2026-08-27  
> 当前源码快照应用清单版本：`0.2.13`；首轮完整候选链因正式前端载荷超出固定上限 670 字节而停止，没有生成安装包。载荷修复后的单独正式构建已低于上限，仍须从新的唯一干净提交完整重跑候选链  
> 发布保护：已发布的 `v0.2.12` 标签、Release、安装包和附件，以及 0.2.11、0.2.10 人工复测候选和更早版本的发布与来源证据保持不可变；0.2.13 真实安装人工复测和单独发布指令前不得创建标签或 Release  
