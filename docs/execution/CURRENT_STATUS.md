...
> 更新日期：2026-08-25  
> 当前源码快照应用清单版本：`0.2.12`；最终本地源码、原生和正式网页端到端门禁已完成，唯一候选提交、推送、标签、安装包和 Release 由后续发布链确认  
> 发布保护：0.2.11 人工复测安装包及来源证据、`0.2.10` 本地人工复测安装包及来源证据，以及 `v0.2.9` 和更早版本的标签、Release、附件、安装包与来源证据保持不可变  
> 历史边界：`v0.2.4` 仍绑定基线提交 `b74d36ef3342db6813d1d43771bc82c0ed2aa1fb`；`v0.2.3` 隐式云调用安全公告继续保留

## 2026-08-25 0.2.12 最终本地门禁完成
...
本节冻结最终本地门禁完成时的源码状态，不预填未来发布结果。唯一干净候选提交、远端 `main`、未签名安装包、标签、GitHub Release、五个附件和回下载复核仍须由后续候选及发布证据确认。准确矩阵、命令和失败链见 [`2026-08-25-V0212-RETEST-RELEASE.md`](2026-08-25-V0212-RETEST-RELEASE.md)。
