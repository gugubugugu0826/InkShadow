...
> 更新日期：2026-08-27  
> 当前发布目标：`0.2.14`；版本清单与源码构建已通过，干净来源候选链、人工门禁、唯一候选提交和安装包尚未完成  
...
| R13-F3-DEDUP     | Data `0079`／Tauri `82` 追加不可变多证据；稳定结构化事实按项目、歧义叙述按章节收敛；作者决定优先。故事核心 148／148，数据层 481／481 | 自动化数据门禁通过；现场数据库未取得                             |
| R13-V1-SELECTION | Data `0080`／Tauri `83` 为新隔离结果冻结改写、润色、扩写或缩写；历史空值保留并明确兼容，四项均覆盖 SQLite、浏览器和重启合同          | 自动化合同通过；真实安装版四项选区与真实模型未执行，仍阻断发布   |
...
当前测试结果为：界面合同 6 个文件、42 项通过；故事核心 148 项通过；数据层 481 项通过；桌面 2,554 项通过、1 项跳过；全仓 514 个文件通过、16 个文件跳过，3,945 项通过、65 项跳过、0 项失败；类型、规范、格式、39 项脚本、敏感信息、137 项许可、20 项包边界、桌面发布配置及 Rust 207 项通过、1 项忽略。`.ink-prose` 界面合同曾被误删，导致首次 `release:check` 在一项界面测试失败后停止；恢复后对应界面测试、最终全仓测试与桌面构建分别重跑通过，但没有取得修复后单次完整 `release:check` 退出 0 的证据。桌面正式前端构建退出码 0：60 个文件、7,337,824／7,340,032 字节，余量 2,208 字节，最大异步分块 494,804／512,000 字节，主 CSS 约 127.36 KB／128 KB。生产 Chromium 26／26 通过；等效 150%／200% 不是真实 Windows 系统缩放。`test:e2e:release` 在捕获来源前按设计拒绝未提交工作区，未启动旅程；完整未签名候选链尚未运行。

真实模型与技能、真人选区、真实安装与覆盖升级、真实触摸、真人视觉、真实 Windows 系统 200% 缩放、原生保存和外部应用打开均未执行。准确矩阵与人工门禁见 [`2026-08-27-V0214-RELEASE-BLOCKERS.md`](2026-08-27-V0214-RELEASE-BLOCKERS.md)。干净来源上的完整发布门禁和上述人工阻断项全部关闭前，不推送、不打标签、不创建 GitHub Release。
