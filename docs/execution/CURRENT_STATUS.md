...
> 更新日期：2026-08-29  
> 当前版本：`0.2.14`；本轮统一缺陷修复继续使用该版本，不修改任何版本来源  
> 当前交付状态：21 项缺陷的生产路径、聚焦回归、全仓测试、原生检查、正式构建和提交前正式网页端到端均已闭合；只剩唯一干净提交上的发布态端到端及远端 `main` 推送后核对  
> 发布保护：现有 `v0.2.14` 及更早标签、Release、安装包和来源证据保持不可变；本轮不创建、移动、覆盖或删除标签、Release 和附件  
> 历史边界：`v0.2.4` 仍绑定基线提交 `b74d36ef3342db6813d1d43771bc82c0ed2aa1fb`；`v0.2.3` 隐式云调用安全公告继续保留

## 2026-08-29 0.2.14 两轮缺陷统一修复（当前）
