...
当前测试结果为：界面合同 6 个文件、42 项通过；故事核心 148 项通过；数据层 481 项通过；桌面 2,554 项通过、1 项跳过；全仓 514 个文件通过、16 个文件跳过，3,945 项通过、65 项跳过、0 项失败；类型、规范、格式、39 项脚本、敏感信息、137 项许可、20 项包边界、桌面发布配置及 Rust 207 项通过、1 项忽略。`.ink-prose` 界面合同曾被误删，首次 `release:check` 因此失败关闭；恢复根因后，唯一干净提交 `b4ec54feb51dc651201daa14e635b261bd9f592c` 上的完整 `release:candidate:unsigned` 退出码为 0。正式前端为 60 个文件、7,337,824／7,340,032 字节，余量 2,208 字节；正式发布端到端 26／26 通过。等效 150%／200% 不是真实 Windows 系统缩放。
