...
> 工程状态：v0.2.6 当前工作树的全量自动化、原生层检查、正式网页构建、体积预算和浏览器端到端回归已通过；隔离 Windows Tauri 真机、真实模型、干净提交、推送、打包和发布仍待完成  
> 历史边界：`v0.2.4` 仍绑定基线提交 `b74d36ef3342db6813d1d43771bc82c0ed2aa1fb`；`v0.2.3` 隐式云调用安全公告继续保留  
> 远端边界：检查时远端 `main` 仍为 `6fd32239b30de9907a0a83b5ce3680e1a3830377`，与本地基线领先/落后均为 0；当前 v0.2.6 修复尚未提交或推送

## 2026-08-21 v0.2.6 提交前状态

本轮从独立真机材料复现并修复 D1–D5：本地数据连接/重载、使用 AI 建议草稿、开书三槽独立结算、
自动备份状态机和能力验证调用账本。应用清单已统一为 `0.2.6`，最新公开版本不变，且未移动
`v0.2.5` 标签。当前结论只绑定共享工作树；在干净唯一提交产生前，不声明 v0.2.6 发布候选或安装包。

| 范围                   | 当前结果                                                                                                                             | 证据边界                                                                                                          |
| ---------------------- | ------------------------------------------------------------------------------------------------------------------------------------ | ----------------------------------------------------------------------------------------------------------------- |
| 全工作区测试           | 通过：472 个测试文件通过、16 个测试文件跳过；3,418 项通过、65 项跳过                                                                 | 桌面端 266 个文件 / 2,095 项通过 / 1 项跳过；网页端 4/33；数据层 70/445                                           |
| 首次失败与复跑         | 开书页 2 项默认 1 秒等待失败 → 仅局部改为 15 秒等待终态 → 聚焦 2/2 → 全量通过；数据层结构化原生错误包装首次 5 项失败 → 修正后 70/445 | 保留失败链，不用局部通过替代最终全量                                                                              |
| 七项静态门禁           | 全部通过：格式 21.38 秒；敏感信息扫描 0；许可证 137；包边界 20；桌面发布配置 17/17；类型检查 20/21；代码规范 0 条警告                | 当前工作树结果；尚未绑定干净提交                                                                                  |
| 原生层                 | 格式通过；严格静态检查首次 1 条测试初始化警告，修复后通过；测试 190 项通过、1 项忽略、0 项失败                                       | 唯一忽略项需要外部本地模型，不计失败                                                                              |
| 正式网页构建与预算     | 2,286 个模块；58 个文件 / 7,068,735 字节；预算计数 7,067,935 / 7,340,032                                                             | 入口 263,379 / 307,200；最大异步块 490,016 / 512,000；样式 128,880 / 131,072；工作线程 1,187,649 / 1,572,864 字节 |
| 浏览器端到端           | 13/17（旧英文定位）→ 16/17（一个遗漏）→ 17/17，最终 26.1 秒                                                                          | 最终定位与普通界面均为中文；不替代真实桌面网页容器                                                                |
...
| 正式发布候选           | 脏工作树清洁门按预期拒绝；发布清单尚未生成                                                                                           | 必须等干净唯一提交后重跑，不保留临时构建为制品                                                                    |
| 隔离真机与真实模型     | 待验证                                                                                                                               | 不把浏览器、临时数据库或回环模型服务写成真机/真实模型结论                                                         |
| 提交、推送、打包和发布 | 待完成                                                                                                                               | 来源指纹、安装包、摘要、标签、远端流水线和公开回下载均不得提前填写                                                |
