...
> 更新日期：2026-08-24  
> 当前工作树应用清单版本：`0.2.9`；截至候选准备时最新公开版本仍为未签名工程预发布 `v0.2.7`；`v0.2.8` 人工复测包继续保留在本地  
...
## 2026-08-24 v0.2.9 真实接口报告修复候选（未发布）

本节编号对应 v0.2.8 真实接口测试报告，不覆盖下方 v0.2.7 历史章节中的同名编号。当前记录聚焦回归、数据层和桌面端全量、静态检查、开发服务器网页旅程、正式构建及原生层最终门禁；标为“独立安全审计”的项目不是测试报告新增缺陷。完整干净提交候选链、安装包和发布均未完成。
