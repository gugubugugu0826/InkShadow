...
> 更新日期：2026-08-23  
> 当前工作树应用清单版本：`0.2.7`；最新公开版本仍为未签名工程预发布 `v0.2.6`  
> 工程状态：`v0.2.7` 源码门禁已通过；初始干净提交候选端到端为 10／17 并按失败关闭，修正后的最终候选仍待从新提交完整重跑；下方 `v0.2.6` 结果全部是不可继承的历史发布证据  
> 历史边界：`v0.2.4` 仍绑定基线提交 `b74d36ef3342db6813d1d43771bc82c0ed2aa1fb`；`v0.2.3` 隐式云调用安全公告继续保留

## 2026-08-23 v0.2.7 当前工作树

本轮以 `v0.2.6` 源码为基线重新实现真正可用的直接模式，并修复正文、版本、方向、故事事实、备份和普通界面问题。附件中的“一次点击自动采用”和取消逐次披露与用户随后直接要求结果可“使用这版／放弃”、且发送前已有逐次披露的反馈冲突，因此以最新直接指令为准：继续逐次披露、结果隔离、明确决定。当前结论是 **实现与当前工作树自动化总门禁通过 / 发布候选与外部验证未完成 / 真实模型需独立授权**，不是发布完成。

| 范围             | 当前结论                                                                               | 证据边界                                                                               |
| ---------------- | -------------------------------------------------------------------------------------- | -------------------------------------------------------------------------------------- |
| 版本             | 工作树清单已升至 `0.2.7`；初始候选提交为 `95904b39a0c2e6c21c30beb0bf02c891dbd798d4`    | 该提交的端到端未全过，不能作为最终候选；尚无标签、安装程序或公开发布                   |
...
| 普通界面         | 直接模式只保留创作首页、作品库和项目内正文、规划、设定、检查；专业术语大幅中文化并隐藏 | 正式网页构建的浅深色、四宽度、键盘与焦点矩阵已过；最终 Tauri 和系统百分之二百缩放待验  |
| 专业模式         | 隔离结果预览、接受/放弃、模型、调用、费用、上下文、任务和诊断继续保留                  | 已进入当前工作树最终总门禁并通过；真实服务与真机交互仍待验证                           |
...
| 工程门禁         | 当前工作树最终 `pnpm release:check` 退出码为 `0`；构建、格式、类型与规范检查全部通过   | 数字只绑定当前工作树；不能继承为尚未形成的干净发布候选证据                             |
| 发布             | 目标是 `v0.2.7` 未签名工程预发布                                                       | 发布候选、安装程序、浏览器端到端、真实外部模型、真机宽度／主题／百分之二百缩放仍待验证 |
...
- 最终 `pnpm release:check` 退出码为 `0`。全工作区 480 个测试文件通过、16 个跳过；3,558 项通过、65 项跳过。
- 桌面端 271 个测试文件通过，2,216 项通过、1 项跳过、0 项失败；网页端 33／33 通过。
- `pnpm build`、格式检查、全工作区类型检查和代码规范检查全部通过。
- 原生层 194 项通过、1 项因显式本地模型条件忽略、0 项失败。
...
- 修正后的最终发布网页构建已完整运行 17 项浏览器旅程并全部通过，用时 1.6 分钟，且生成 40 张唯一视觉证据；最终结论仍必须等待新干净提交从源码门禁开始完整重跑候选链。
- 干净提交 `ddd121b517ac32a0aacc5f63a3f1ebca3b457b64` 的完整候选首跑在桌面端全量出现 1 项旧节点竞态：270 个文件、2,215 项通过，1 个文件／1 项失败，1 项跳过；候选在打包前停止。修正后设置页 58／58 通过，最终候选仍须新提交从头重跑。
- 以上全工作区数字绑定初始提交及其源码门禁；最终安装程序、完整浏览器端到端、真实外部模型，以及系统百分之二百缩放仍待验证。

当前缺陷矩阵、迁移、聚焦结果、待验证项和发布哈希占位见
...
- 本地 Community 创作工程闭环和未签名 `v0.2.6` 工程预发布已公开；这不等于 Windows 商业发布获批。
