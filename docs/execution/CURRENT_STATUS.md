...
> 更新日期：2026-08-21  
> 当前应用清单版本：`0.2.6`；最新已公开版本仍为 `v0.2.5`，其标签解析后指向唯一来源提交 `5b3e212cafde10cd75fa87b7b74bfdfff9347a3d`  
> 工程状态：v0.2.6 的 D1–D5 已在隔离 Windows Tauri 程序完成聚焦复测；当前修复源码提交为 `722e67e`，包含本报告的干净候选提交仍待创建并完成全量验证、真实外部模型、推送、打包和发布  
> 历史边界：`v0.2.4` 仍绑定基线提交 `b74d36ef3342db6813d1d43771bc82c0ed2aa1fb`；`v0.2.3` 隐式云调用安全公告继续保留  
> 远端边界：远端 `main` 仍为 `6fd32239b30de9907a0a83b5ce3680e1a3830377`；本地修复链 `374b3ab` → `4ffa1ba` → `722e67e` 领先 3 个提交，尚未推送

## 2026-08-21 v0.2.6 真机复测后、候选前状态

本轮从独立真机材料复现并修复 D1–D5：本地数据连接/重载、使用 AI 建议草稿、开书三槽独立结算、
自动备份状态机和能力验证调用账本。应用清单已统一为 `0.2.6`，最新公开版本不变，且未移动
`v0.2.5` 标签。聚焦真机结论绑定 `722e67e` 的隔离可执行程序；最终全量候选尚未运行，
不声明 v0.2.6 发布候选或安装包。

| 范围               | 当前结果                                                                                                                             | 证据边界                                                                                                                                                 |
| ------------------ | ------------------------------------------------------------------------------------------------------------------------------------ | -------------------------------------------------------------------------------------------------------------------------------------------------------- |
| 全工作区测试       | `374b3ab` 上通过：472 个测试文件通过、16 个测试文件跳过；3,418 项通过、65 项跳过                                                     | 当前修复源码 `722e67e` 后尚未重跑；包含本报告的干净候选创建后须重跑。桌面端 266/2,095+1 跳过、网页端 4/33、数据层 70/445 是较早的完整快照                |
| 首次失败与复跑     | 开书页 2 项默认 1 秒等待失败 → 仅局部改为 15 秒等待终态 → 聚焦 2/2 → 全量通过；数据层结构化原生错误包装首次 5 项失败 → 修正后 70/445 | 保留失败链，不用局部通过替代最终全量                                                                                                                     |
| 七项静态门禁       | `374b3ab` 上全部通过：格式 21.38 秒；敏感信息 0；许可证 137；包边界 20；发布配置 17/17；类型 20/21；规范 0 警告                      | 当前修复源码 `722e67e` 后尚未完整重跑；干净候选创建后须完整重跑                                                                                          |
| 原生层             | `4ffa1ba` 上格式、严格静态检查和 190 项通过 / 1 项忽略；`722e67e` 的回执差量为聚焦 19/19                                             | 包含本报告的干净候选提交尚未创建，原生层全量尚未在该候选上重跑                                                                                           |
| 正式网页构建与预算 | `374b3ab`：2,286 个模块；58 个文件 / 7,068,735 字节；预算计数 7,067,935 / 7,340,032                                                  | 最终提交后尚未重跑；入口 263,379、最大异步块 490,016、样式 128,880、工作线程 1,187,649 字节                                                              |
| 浏览器端到端       | `374b3ab`：13/17（旧英文定位）→ 16/17（一个遗漏）→ 17/17，最终 26.1 秒                                                               | 最终提交后尚未重跑；普通界面定位已为中文                                                                                                                 |
| 凭据隔离           | 凭据命名空间从 Tauri 应用标识派生；正式标识 `com.inkshadow.desktop` 的值不变                                                         | 隔离回归覆盖模型、设备、项目、云端和更新凭据；不迁移、复制或删除既有凭据                                                                                 |
| 数据变更           | 新增前向迁移 `0071_model_capability_probe_invocation_ledger.sql`，桌面原生内部版本升至 `74`                                          | 不修改已发布迁移或摘要；自动备份清单升为文件系统格式第 2 版，不是数据库迁移                                                                              |
| 隔离 Windows Tauri | D1–D5 聚焦复测完成；程序 25,886,208 字节，SHA-256 `2c0e1da5…f0a139`                                                                  | 绑定 `722e67e`、独立标识/数据/凭据；没有生成安装包                                                                                                       |
| 自动备份真机       | 非空作者数据备份 3,653,632 字节、清单修订 5 成功；172 张表逐表差异 0，昨日两份健康文件保留                                           | 正式页面接受后为作品 4 / 章节 6 / 版本 7 / 建议草稿 1 / 任务 1 / 模型调用记录 4 / 旅程 2 / 问答轮次 9，调用增量 0；权限/磁盘/竞争/写入中强停真机仍待完成 |
| 回环与真实外部模型 | 回环 4 次逐条入账：7/1、321/512、未知/未知、未知/未知词元；费用均未知；真实外部模型 0 次、词元 0、费用 0                             | 真实外部模型为外部条件阻塞，不把回环服务冒充外部模型                                                                                                     |
| 正式发布候选       | 早期脏工作树清洁门按预期拒绝；最终发布清单尚未生成                                                                                   | 必须从最终干净唯一提交完整重跑，不保留隔离程序为发布制品                                                                                                 |
| 推送、打包和发布   | 待完成                                                                                                                               | 本地修复链领先 3；来源指纹、安装包、摘要、标签、远端流水线和公开回下载均不得提前填写                                                                     |
...
[`RELEASE_CHECKLIST.md`](RELEASE_CHECKLIST.md)。下方 v0.2.5 及更早内容按原日期保留，不倒改为
v0.2.6 证据。
