...
> 更新日期：2026-08-28  
> 当前版本：`0.2.14`；完整未签名候选链已通过，最新用户指令已授权在最终同源复核与远端持续集成通过后发布 GitHub 预发行  
...
此前测试结果为：界面合同 6 个文件、42 项通过；故事核心 148 项通过；数据层 481 项通过；桌面 2,554 项通过、1 项跳过；全仓 514 个文件通过、16 个文件跳过，3,945 项通过、65 项跳过、0 项失败；类型、规范、格式、39 项脚本、敏感信息、137 项许可、20 项包边界、桌面发布配置及 Rust 207 项通过、1 项忽略。唯一干净提交 `b4ec54feb51dc651201daa14e635b261bd9f592c` 上的完整 `release:candidate:unsigned` 退出码为 0。最终文案提交后的首次同源候选链在故事核心四万字性能断言处失败关闭（约 2.23 秒，大于 2 秒）；根因是 `0079` 触发器重复逐字符扫描整篇正文。新增只向前 `0081` 后，同一用例约 0.75 秒，故事核心 148／148、数据迁移恢复 52／52、原生迁移 18／18 已通过；最终同源完整候选链不得用此前结果替代。等效 150%／200% 不是真实 Windows 系统缩放。
