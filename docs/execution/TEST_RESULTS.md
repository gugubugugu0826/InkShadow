...
> 当前发布目标：`0.2.14`。本节只记录已经运行的本轮命令；完整门禁尚未结束，历史数字不得替代当前结果。
...
| 完整发布检查       | `pnpm release:check`                                                         | 分项复跑通过 |    1 |                0 | 首次在 1 项界面合同失败后停止；修复后 UI、全仓测试与桌面构建分别重跑通过，但未取得一条修复后完整退出 0 的记录                    |
| 正式发布端到端     | `pnpm test:e2e:release`                                                      |            0 |    1 |                0 | 捕获来源前按设计拒绝未提交工作区，未启动 E2E；必须在唯一干净提交后重跑                                                           |
| 完整未签名候选链   | `pnpm release:candidate:unsigned`                                            |            — |    — |                — | 尚未运行；必须先形成唯一干净提交                                                                                                 |

本轮曾出现并保留以下失败链：数据层全量首次为 476 项通过、5 项失败，原因是两个现代测试夹具遗漏 `0080`；补齐完整迁移链后为 481／481。桌面全量首次为 2,549 项通过、2 项失败、1 项跳过：长篇基准夹具停在旧迁移上限，原子追踪提交错误又被误分到供应商部分结果恢复；修正根因并纳入最终工作树后为 2,554 项通过、1 项跳过。Rust 首次有两项过时迁移上限断言，更新到 Tauri `83` 后为 207 项通过、1 项忽略。首次 `release:check` 在 UI 一项合同失败后停止，准确根因是 `.ink-prose` 界面合同被误删；恢复合同后 UI 6 个文件、42／42、最终全仓 3,945／3,945 和桌面正式构建分别重跑通过。上述修正没有删除断言或放宽固定包体预算，也没有伪造一条修复后完整 `release:check` 退出 0 的记录。

当前桌面正式构建已在固定预算内通过，生产 Chromium 端到端为 26／26；但 `test:e2e:release` 因工作区未提交，在捕获发布来源前按设计停止且没有启动任何旅程。它必须在唯一干净提交后重跑，完整未签名候选链仍未运行，不能把分项通过写成候选准入完成。
