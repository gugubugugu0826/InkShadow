...
> 更新日期：2026-08-21  
> 证据范围：当前应用清单版本为 `0.2.6`，最新公开标签仍为 `v0.2.5`。顶部区分首次修复提交的全量自动化、两次真机补修和 `722e67e` 隔离 Windows Tauri 聚焦结果；最终全量候选、真实外部模型、推送、打包和发布不会提前标为通过。下方 v0.2.5 及更早结果按原日期保留为历史快照。

## 2026-08-21 v0.2.6 分层门禁与隔离真机

状态：D1–D5 隔离 Windows Tauri 聚焦复测完成；`722e67e` 是当前修复源码提交，包含本报告的干净候选
提交仍待创建，其全量验证、真实外部模型和正式发布链待运行。

当前应用清单已统一为 `0.2.6`。修复源码形成 `374b3ab` → `4ffa1ba` → `722e67e` 三个本地提交；
完整自动化数字主要绑定首个提交，原生命令栈修复的全量绑定第二个提交，最新回执差量目前只有聚焦
19/19。正式发布候选必须从包含本报告的干净唯一提交重新完整运行。远端 `main` 仍为
`6fd32239b30de9907a0a83b5ce3680e1a3830377`，本地领先 3 个提交且尚未推送。

### 全工作区与分项结果

| 范围                         | 精确命令                                                                                      | 当前分层结果                                                                |
| ---------------------------- | --------------------------------------------------------------------------------------------- | --------------------------------------------------------------------------- |
| 全工作区测试                 | `pnpm test`                                                                                   | `374b3ab`：退出码 0；472 个文件通过、16 个文件跳过；3,418 项通过、65 项跳过 |
...
| 格式                         | `pnpm format:check`                                                                           | 退出码 0；21.38 秒                                                          |
| 敏感信息扫描                 | `pnpm check:secrets`                                                                          | 退出码 0；发现数 0                                                          |
| 许可证                       | `pnpm check:licenses`                                                                         | 退出码 0；137 项                                                            |
| 包边界                       | `pnpm check:boundaries`                                                                       | 退出码 0；20 个包                                                           |
| 桌面发布配置                 | `pnpm check:desktop-release`                                                                  | 退出码 0；17/17                                                             |
| 全工作区类型检查             | `pnpm typecheck`                                                                              | 退出码 0；20/21 个含脚本的项目通过                                          |
| 代码规范                     | `pnpm lint`                                                                                   | 退出码 0；0 条警告                                                          |
| 原生层格式                   | `cargo fmt --manifest-path apps/desktop/src-tauri/Cargo.toml -- --check`                      | 退出码 0                                                                    |
| 原生层严格静态检查           | `cargo clippy --manifest-path apps/desktop/src-tauri/Cargo.toml --all-targets -- -D warnings` | 修复 1 条测试初始化警告后退出码 0                                           |
| 原生层测试                   | `cargo test --manifest-path apps/desktop/src-tauri/Cargo.toml`                                | `4ffa1ba`：190 项通过、1 项忽略、0 项失败；`722e67e`：聚焦 19/19            |

七项静态门禁指格式、敏感信息、许可证、包边界、桌面发布配置、类型检查和代码规范；它们在
`374b3ab` 上均为绿色。全工作区数字来自一次串行工作区运行，不与聚焦结果重复相加，也不冒充
`722e67e` 的最终全量结果。
...
### `374b3ab` 正式网页构建、体积与浏览器端到端

提交 `374b3ab` 的正式网页构建完成 2,286 个模块。输出与预算如下；所有上限保持不变。
...
浏览器端到端在该提交上最终为 17/17，通过用时 26.1 秒，且普通界面定位已经全部改为中文。
该结果只证明当时的正式网页构建；它不替代 `722e67e` 后的最终重跑、Windows Tauri 桌面网页容器、
系统 200% 显示缩放或真实安装流程。

正式发布构建在脏工作树清洁门按预期停止，因此当前没有可发布清单、来源指纹、安装包或摘要。
这项停止防止把未提交工作树包装成制品，不计为产品回归失败，也不得改写成发布通过。
...
- 创建包含本报告的干净唯一候选提交，并在其上运行全量工作区、七项静态门禁、原生层全量、
  正式网页构建、体积预算、浏览器端到端和发布候选链。
- D4 权限拒绝、磁盘写入失败、目标竞争和写入中强制结束的最终安装包真机故障模式。
- 真实外部模型回归；预算已声明为最多 4 次、最多 20,000 合计词元、费用不超过人民币 2 元，
  但隔离环境没有可用外部凭据，因此实际为 0 次。
- 最终干净唯一提交、推送、来源指纹、未签名安装包、发布清单、SHA-256 摘要、标签、远端流水线、
  公开发布和附件回下载。
