...
> 证据范围：当前清单为 0.2.9，本节记录候选聚焦回归、数据层和桌面端全量、静态检查、开发服务器网页旅程、正式构建及原生层最终门禁；0.2.8 本地人工复测包继续保留，最新公开 `v0.2.7` 的唯一干净候选、标签和附件保持不可变。完整干净提交候选链、真实外部服务和真实安装程序人工矩阵均未完成，历史内容按原日期保留。

## 2026-08-24 v0.2.9 报告缺陷与当前自动化结果（候选未发布）

C1、C2 的根因来自现场页面证据与当前源码的联合核对：均可在当前源码稳定复现并与现场现象一致，但缺少的现场日志、能力和路由快照意味着它们不能被写成现场唯一根因。表中标为“独立安全审计”的两项不是报告新增缺陷。
