...
> 证据范围：当前源码快照应用清单为 `0.2.12`；最终本地源码、原生与正式网页端到端门禁已经完成。唯一候选提交、推送、标签、安装包或 Release 不属于下表已完成范围，仍由后续候选与发布证据确认。0.2.11 及更早候选和发布证据保持不可变。

## 2026-08-25 0.2.12 最终本地门禁
...
专项矩阵、数据守恒和未执行边界见 [`2026-08-25-V0212-RETEST-RELEASE.md`](2026-08-25-V0212-RETEST-RELEASE.md)。本页是最终本地门禁源码快照；干净提交上的 `pnpm release:candidate:unsigned`、远端持续集成、安装包、标签、GitHub Release 和附件回下载仍待后续发布链执行，不能由上述结果代替。
