...
> 证据范围：当前源码快照应用清单为 `0.2.13`；本页顶部只记录本轮当前运行证据。桌面端完整测试、全仓测试、类型、规范、构建和正式网页端到端已取得当前结果；唯一干净提交、正式候选端到端和完整未签名候选链仍待补录。已发布 v0.2.12 和更早候选、标签、Release 与附件保持不可变。
...
| `pnpm typecheck`／`pnpm build`                                                                                                               |           通过 |    0 |          0 | 均为 20／21 个有脚本的工作区；构建包含桌面固定包体门禁        |
...
- 固定网页包体上限为 7,340,032 字节；本轮多次在不提高预算的前提下从超出 1,172 字节逐步收敛，最终当前源码为 7,340,007 字节，余量 25 字节。
...
专项矩阵、数据守恒和未执行边界见 [`2026-08-25-V0212-RETEST-RELEASE.md`](2026-08-25-V0212-RETEST-RELEASE.md)。本节是当时最终本地门禁源码快照；其中候选链和发布“待执行”只描述该历史时点。v0.2.12 后续已经发布并固定解析到 `8d20dfbbdac1eaecdde046714ba257257cd68ace`，其标签、Release 和附件保持不可变；上述源码门禁不能替代当前 0.2.13 结果。
