...
> 证据范围：当前工作树应用清单为 `0.2.7`，最新公开标签仍为不可移动的 `v0.2.6`。本轮完整源码门禁、全工作区测试、正式构建与原生层检查均已通过；初始发布端到端已实际运行但未全过，最终候选复跑、未签名打包、真实外部服务和最终安装程序真机流程仍不得继承旧结果。后续 `v0.2.6` 及更早内容按原日期保留为历史快照。

## 2026-08-23 v0.2.7 当前工作树门禁结果

状态：`源码门禁通过 / 初始候选端到端 10／17 并按失败关闭 / 修正后工作树端到端 17／17 / 最终候选待完整重跑 / 真实外部服务未授权 / 尚未发布`。
...
修正后的最终发布网页构建执行 `CI=true node scripts/run-e2e.mjs --dist apps/desktop/dist-release`，17／17 通过，用时 1.6 分钟；其中本地导入导出完整单文件 4／4、响应式两项 2／2、模型中心与视觉证据 4／4 均通过，并生成 40 张唯一图片。该工作树全量结果仍不能替代新干净提交上的完整候选链。
...
这次定向通过不能替代新干净提交的完整候选链。

以下项目仍待本轮最终候选执行或真实环境验证：

- 最终干净提交上的 `pnpm test:e2e:release`
