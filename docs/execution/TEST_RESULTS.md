...
> 证据范围：当前工作树应用清单为 `0.2.7`，最新公开标签仍为不可移动的 `v0.2.6`。本轮完整源码门禁、全工作区测试、正式构建与原生层检查均已通过；发布端到端、未签名候选打包、候选复核、真实外部服务和最终安装程序真机流程仍不得继承旧结果。后续 `v0.2.6` 及更早内容按原日期保留为历史快照。

## 2026-08-23 v0.2.7 当前工作树门禁结果

状态：`完整源码门禁通过 / 发布候选链尚未完成 / 真实外部服务未授权 / 尚未发布`。

### 当前最终证据

| 当前范围                       | 精确命令或来源               | 本轮最终结果                                                     | 不能外推的范围                           |
| ------------------------------ | ---------------------------- | ---------------------------------------------------------------- | ---------------------------------------- |
| 完整源码门禁                   | `pnpm release:check`         | 退出码 0；测试、静态检查和正式构建全部通过                       | 发布端到端、打包、公开发布和真实外部服务 |
| N-NEW1 至 N-NEW5 核心回归      | 五个核心测试文件             | 193／193 通过                                                    | 真实受影响数据库、真实外部服务和真机重开 |
| 全工作区测试                   | `pnpm test`                  | 480 个文件通过、16 个文件跳过；3,558 项通过、65 项跳过           | 被跳过的显式外部条件与真机人工流程       |
| 桌面端                         | 全工作区同一次桌面测试脚本   | 271 个文件；2,216 项通过、1 项跳过、0 项失败                     | 真实模型、最终安装程序和系统级交互       |
| 数据层                         | 全工作区同一次数据层测试脚本 | 73 个文件；453／453 通过                                         | 真实历史库安装升级和最终安装程序恢复     |
| 网页端                         | 全工作区同一次网页测试脚本   | 4 个文件；33／33 通过                                            | 桌面原生壳、系统文件选择器和真机显示缩放 |
| 正式构建                       | `pnpm build`                 | 通过；网页端转换 1,616 个模块、桌面端转换 2,289 个模块；预算通过 | 安装程序启动、升级、卸载和签名           |
| 原生层                         | `pnpm check:rust`            | 194 项通过、1 项因显式本地模型条件忽略、0 项失败                 | 未配置的本地模型外部条件和最终安装真机   |
| 格式、类型、代码规范与差异检查 | 当前工作树最终检查           | 全部通过                                                         | 不替代端到端、安装程序和外部服务验证     |

全工作区数字来自最终 `pnpm release:check` 的一次完整运行，不与聚焦结果重复相加。原生层唯一忽略项要求显式配置本地模型服务，未被记为通过。
...
- 真实外部服务，以及最终安装程序的安装、启动、升级、卸载、重装、迁移、恢复和真机人工检查

真实模型的开头、直接续写、系统方向续写和自定义方向续写均为零次；未读取保存凭据，未产生费用。Windows 系统百分之二百缩放也未在本轮验证。详见
[2026-08-22-V027-DIRECT-MODE-REMEDIATION.md](2026-08-22-V027-DIRECT-MODE-REMEDIATION.md)。

以下 `v0.2.6` 结果是历史发布证据，不是 `v0.2.7` 当前工作树结果。
