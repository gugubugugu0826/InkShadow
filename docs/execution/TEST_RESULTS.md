...
> 更新日期：2026-08-21  
> 证据范围：当前应用清单版本为 `0.2.6`，最新公开标签仍为 `v0.2.5`。顶部记录 v0.2.6 当前工作树的自动化结果；尚未完成的隔离 Windows Tauri 真机、真实模型、干净提交、推送、打包和发布不会标为通过。下方 v0.2.5 及更早结果按原日期保留为历史快照。

## 2026-08-21 v0.2.6 当前工作树门禁

状态：全量自动化通过；隔离真机、真实模型和正式发布链待验证。

当前应用清单已统一为 `0.2.6`，但这些结果尚未绑定干净唯一提交。正式发布候选命令在脏工作树
清洁门按预期拒绝继续，发布清单必须等提交后重新生成。检查时远端 `main` 仍为
`6fd32239b30de9907a0a83b5ce3680e1a3830377`，与本地基线领先/落后均为 0；当前修复未推送。

### 全工作区与分项结果

| 范围                         | 精确命令                                                                                      | 最终结果                                                                 |
| ---------------------------- | --------------------------------------------------------------------------------------------- | ------------------------------------------------------------------------ |
| 全工作区测试                 | `pnpm test`                                                                                   | 退出码 0；472 个测试文件通过、16 个测试文件跳过；3,418 项通过、65 项跳过 |
...
| 原生层测试                   | `cargo test --manifest-path apps/desktop/src-tauri/Cargo.toml`                                | 190 项通过、1 项忽略、0 项失败；忽略项为显式外部本地模型用例             |

七项静态门禁指格式、敏感信息、许可证、包边界、桌面发布配置、类型检查和代码规范；七项最终均为绿色。
全工作区数字来自一次串行工作区运行，不与聚焦结果重复相加。
...
生产 Vite 构建完成 2,286 个模块。输出与预算如下；所有上限保持不变。
...
浏览器端到端最终为 17/17，通过用时 26.1 秒，且普通界面定位已经全部改为中文。该结果只证明
正式网页构建；它不替代 Windows Tauri 桌面网页容器、系统 200% 显示缩放或真实安装流程。
...
- 真正隔离桌面进程中的五类凭据路径仍属于待执行的 Tauri 真机回归，不用纯函数与原生层自动化替代。

### 尚未验证

- 隔离应用标识下的 Windows Tauri D1–D5 真机回归，包括连续重载、锁竞争、强制结束/重启和
  独立备份恢复。
- 真实模型预算声明、三槽开书与一次固定能力验证、精确输入/输出词元、费用和本地调用账本逐条对账。
- 干净唯一提交、推送、来源指纹、未签名安装包、发布清单、SHA-256 摘要、标签、远端流水线、
  公开发布和附件回下载。
