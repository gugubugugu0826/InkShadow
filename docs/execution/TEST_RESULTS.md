...
> 更新日期：2026-08-20  
> 证据范围：当前应用清单版本为 `0.2.5`，最新已发布标签 `v0.2.4` 指向基线提交 `b74d36ef3342db6813d1d43771bc82c0ed2aa1fb`；当前未提交工作树是 v0.2.5 Pre-release 候选。下方旧版本结果按原日期保留为历史快照，不能作为当前工作树证据。当前工作树的 Provider live 与真实 Windows Tauri 均为 `NOT_RUN / NOT_RETESTED`。

## 2026-08-20 v0.2.5 候选最终门禁

状态：`PENDING_FINAL_RUN`。

当前源码实现已经完成收口，但仍处于未提交工作树；下方记录的分项与当前完整门禁不能当作干净唯一
提交的最终候选证据。最终必须
在工作树停止变化后补录同一快照的精确命令、退出码、bundle 字节和结果，至少包括：受影响单元/集成、真实临时 SQLite、
Data `0066`–`0070`、Tauri `69`–`73`、备份恢复、SQLite reload、StoryMemory/Narrative/多粒度 FTS、
直接/专业模式、Candidate/版本、Production Agent/TaskGraph/prompt injection、Provider 动作披露、
四格式图片导出/原生保存回执、Desktop TypeScript、ESLint、Prettier、
`git diff --check`、production build、bundle graph、Playwright、静态视觉 manifest/checker 和完整
`release:check`。production 长篇 benchmark 只能在唯一冻结 commit 存在后，使用同一
40 位 source revision 生成原始 JSON 与汇总；当前明确为 `FINAL_BENCHMARK_PENDING`。

若完整 `release:check` 仅因工作树不是干净唯一提交而拒绝，应原样记录为预期发布门禁，不得为此
提交、隐藏改动或移除检查。真实 Provider 保持 `NOT_RUN`，当前工作树的真实 Tauri/Windows 系统
200% DPI 保持 `NOT_RETESTED`。独立实测使用
[`NEXT-UNRELEASED-WINDOWS-TAURI-PHASE-2-PROMPT.md`](NEXT-UNRELEASED-WINDOWS-TAURI-PHASE-2-PROMPT.md)。

### 当前已完成的分项证据（尚非同一冻结候选）

工作树已停止功能性写入，下列分项结果仍只证明各自执行时的代码切片；最终全链仍为
`PENDING_FINAL_RUN`，不能把这些分项相加成 Release PASS。
除表内另有说明，当前命令 cwd 均为 `D:\InkShadow`，且没有使用真实 Provider/Key。当前分项
交接没有保留统一 duration 或冻结 commit；不得补造，最终冻结运行必须逐项记录 duration、commit、
exit code、passed/failed/skipped 与失败后的复跑。
...
| 开书 Provider 精确披露            | `.\node_modules\.bin\vitest.cmd run --config apps/desktop/vitest.config.ts --configLoader runner apps/desktop/src/pages/idea-journey-page.test.tsx apps/desktop/src/infrastructure/model-hub-creative-chain-integration.test.ts`                                                                                                                                                                                                                                                                                                           | PASS；exit 0；2 files / 79 tests；44.88s                                                                                                                                                    | 四类开书动作均覆盖二次确认、取消 0-call，以及 route/cost/source/privacy 的最终 Provider 边界复核；未使用真实 Provider/Key，最终全量 Desktop 仍 `PENDING_FINAL_RUN`。                                                                                                                                                                                                                                                                                                                         |
...
| Production 长篇 benchmark         | `INKSHADOW_SOURCE_REVISION=<frozen-40-hex> pnpm benchmark:long-form:production`                                                                                                                                                                                                                                                                                                                                                                                                                                                            | `FINAL_BENCHMARK_PENDING`                                                                                                                                                                   | runner 已接线 5k/20k/50k/200k、≥30 样本、真实临时 SQLite 与当前 production consumers；必须等唯一冻结 commit 后才允许生成 commit-bound 原始结果。                                                                                                                                                                                                                                                                                                                                             |
...
598.36 秒。唯一 skipped 仍是显式外部依赖项；没有真实 Provider/Key。该结果属于当前未提交工作树，
最终 `release:check` 和干净唯一提交候选仍须重跑，不能提前外推为 Release PASS。

当前工作树 production build 使用：
...
bytes。所有既有上限保持不变。该数字仍是未提交工作树证据，冻结候选必须重新构建并以 manifest
为准。
...
断言。定向 3/3 后，同一完整命令复跑为 17/17 PASS，26.3 秒。视觉 checker 随后为 32 records /
32 unique PNG PASS；manifest 仍诚实记录 dirty baseline、static Chromium、真实 Tauri 与系统 DPI 未运行。
...
这证明当前工作树已经通过源码级完整门禁，但还不是干净唯一 commit，也未运行 clean-HEAD
`release:candidate:unsigned`，因此顶部状态仍保持 `PENDING_FINAL_RUN`，不得提前写为已发布。
