...
> 更新日期：2026-08-11  
> 证据范围：本页保留各次命令对应的历史证据。当前发布目标为 `v0.2.2`；本节工程数字绑定提交前工作树快照，仍须由唯一干净提交的正式候选链重新确认。真实 DeepSeek、当前 Tauri WebView、192 次付费 A/B 与 2,496 项人工评分保持 `NOT_RUN`。此前 clean-worktree 安全探针曾按设计拒绝脏工作树；`v0.2.2` 唯一提交尚未生成，正式候选与 GitHub Pre-release 尚未运行。

## 2026-08-11 v0.2.2 提交前工作树工程门禁
...
| v0.2.2 唯一提交         | NOT_RUN                                                                                                                              | 版本与文档正在收口；Commit SHA 与源码指纹尚未生成                                                                                                                                                 | IN_PROGRESS |
| v0.2.2 干净候选链       | `pnpm.cmd test:e2e:release`                                                                                                          | 正式候选尚未运行；此前同一命令在任何构建或 E2E 前拒绝脏工作树，只证明 clean-worktree gate 有效，不是 v0.2.2 候选证据                                                                              | NOT_RUN     |
| GitHub v0.2.2 Release   | NOT_RUN                                                                                                                              | 标签、Release、安装包、manifest 与 SHA 附件均未创建或发布                                                                                                                                         | NOT_RUN     |
...
上述 Chromium 证据不是 Tauri WebView、安装包、真实 DeepSeek 或真实付费评测证据。`test:e2e:release` 的历史 clean-worktree 拒绝是候选来源安全门正常工作，不是代码测试失败；它也不能记作 v0.2.2 正式候选已运行。公开 `v0.2.1` 是不可覆盖的历史未签名 Pre-release：<https://github.com/gugubugugu0826/InkShadow/releases/tag/v0.2.1>。
