...
> 当前交付目标：`0.2.18`。完整本地门禁通过后已获安全推送 main、创建新标签、发布、回下载和清理的授权。未签名包只能预发行；旧标签、附件和 0.2.17 人工复测包保持不变。

## 0.2.18 当前门禁

| 范围         | 必须满足的条件                                                                                                                                              |
| ------------ | ----------------------------------------------------------------------------------------------------------------------------------------------------------- |
| 修复         | 五项发布阻断与八项高优先级均有生产根因、失败回归和当前结果；现场材料未取得时如实说明                                                                        |
| 数据与发送   | 权威回读、原子版本、结果隔离、私密最终边界零发送、取消零发送、结果不确定保留记录且零自动重试                                                                |
| 全量验证     | 完整候选链涵盖构建、格式、敏感信息、许可证、架构、类型、代码规范、脚本、全工作区测试、原生严格检查、正式浏览器与真实文件导出                                |
| 视觉与人工   | 四个目标宽度及等效 200% 自动化；真实服务、真人视觉／触摸、系统缩放、安装升级与现场库未执行时明确标注                                                        |
| 唯一来源     | 所有步骤绑定同一干净提交；来源变更后重新完整验证，不拿前一提交结果充当当前证明                                                                              |
| 安装包       | `InkShadow_0.2.18_x64-setup.exe`，产品和文件版本规范化均为 0.2.18，应用载荷为 AMD64；未签名明确标为工程预览                                                 |
| 五项附件     | 安装包、`InkShadow_0.2.18_SOURCE_MANIFEST.json`、`SHA256SUMS.txt`、`InkShadow_0.2.18_VALIDATION_REPORT_zh-CN.md`、`InkShadow_0.2.18_RELEASE_NOTES_zh-CN.md` |
| 远端         | 安全同步、不强推、不移动旧标签；main、新说明标签、Release 与来源清单指向同一提交；未签名 Release 必须预发行                                                 |
| 回下载与清理 | 回下载每项附件核对大小和 SHA-256 后再清理；保留证据、安装包及清单，不清空数据库或删除作品                                                                   |

完整缺陷矩阵见 [本轮执行记录](2026-09-06-V0218-REPAIR.md)。本表是准入要求，不预填未产生的通过结论。以下旧版本门禁保留为历史，旧授权限制不适用于本轮。

## 0.2.17 人工复测候选门禁（历史）

| 门禁         | 准入要求                                                                                                                                                                                                                                   |
| ------------ | ------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------ |
| 编辑器状态   | “生成续写建议”常驻；四项选区工具只随当前有效选区出现。续写与选区任务按项目、章节、正文版本、任务、选区和会话隔离草稿，迟到恢复不得覆盖当前状态，成功、失败、取消和离页均有确定终态。                                                       |
| 写作交互     | 普通界面每次任务只显示一个“本次要求（可选）”；篇幅只显示短、中、长及自然停止条件。精确目标只在高级设置显示并明确允许约正负百分之二十浮动，不得承诺固定字数。                                                                               |
| 提示词与输出 | 全部生产任务遵守安全与隐私、正文与锁定事实、任务、作者本次要求、规划与设定、技能、偏好、默认风格的统一优先级。正文输出只允许可审阅小说正文；结构化任务使用独立契约，解析或边界检查失败必须诚实结算并保持结果隔离。                         |
| 资料与技能   | 必需资料先保留预算，最新正文不得被低优先级资料挤出；来源、版本、定位和内容指纹共同去重，并记录采用、省略、原因和预算。自定义技能必须经过任务匹配、实际编译、发送前快照和调用记录证明采用，不能把“已启用”冒充“已采用”。                     |
| 私密发送     | 私密章节在正文、规划、摘要、审稿、一致性、远程向量、后台整理和自动任务的真实发送边界前统一失败关闭；本地整理可以继续并明确标为本地执行。确认前取消为零发送，发送后不确定保留调用事实，自动重试为零。                                       |
| 数据安全     | 正文、不可变版本、灵感、规划、设定、历史候选和调用审计摘要保持不变；当前迁移上限仍为 Data `0082`／Tauri `85`，本轮不新增迁移，也不修改任何已发布迁移字节或校验值。                                                                         |
| 完整自动化   | 完整数据、桌面、类型、规范、格式、架构、敏感信息、正式构建、Rust 和正式浏览器旅程全部退出 0；提示词快照、输出契约、编辑器重启恢复、资料竞争预算和所有私密路线零发送均须纳入当前提交证据。                                                  |
| 人工边界     | 未执行的真实供应商、真实安装、系统级百分之二百缩放、触摸与真人视觉明确披露，不得由模拟服务、临时 SQLite 或自动化等效缩放替代。                                                                                                             |
| 唯一来源     | 候选来自一个干净、唯一、可追踪提交；构建期间提交、索引、源码指纹和正式前端指纹不得变化。                                                                                                                                                   |
| 安装包       | 人工复测安装包名为 `InkShadow_0.2.17_x64-setup.exe`；ProductName、FileDescription、ProductVersion 与 FileVersion 准确，载荷为 AMD64。未签名时 Authenticode 必须如实记录为 NotSigned，并标明“未签名工程预览”。                              |
| 五项本地附件 | 只保留 `InkShadow_0.2.17_x64-setup.exe`、`InkShadow_0.2.17_SOURCE_MANIFEST.json`、`SHA256SUMS.txt`、`InkShadow_0.2.17_VALIDATION_REPORT_zh-CN.md` 和 `InkShadow_0.2.17_MANUAL_RETEST_GUIDE_zh-CN.md`；名称、大小、摘要和来源提交必须一致。 |
| 远端边界     | 本轮在本地候选提交、附件复核与可再生资源清理后停止。真实安装人工复测完成并取得用户单独发布指令前，不得推送当前候选、创建正式标签、GitHub Release、草稿 Release 或公开附件。                                                                |

本轮实际命令、统计、未执行边界、候选来源和人工复测附件事实记录在 [`2026-09-01-V0217-CANDIDATE.md`](2026-09-01-V0217-CANDIDATE.md) 中。未知提交、总数、摘要和路径不得预填。

## 0.2.16 未签名预发行门禁（历史）

| 门禁       | 准入要求                                                                                                                                                                                                                                                           |
| ---------- | ------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------ |
| 生产缺陷   | F17、F19、F20、F21、F22、F23、F24 和本轮体验项必须绑定修复前失败证据、准确生产根因与修复后回归；F17 失败后必须可安全离开                                                                                                                                           |
| 数据安全   | 正文、不可变版本、灵感、设定、历史候选、章节隐私和任务审计摘要保持不变；连续升级、备份与恢复覆盖全部用户表                                                                                                                                                         |
| 发送安全   | 私密章节零发送；选区操作确认前零发送，发送后结果诚实记录，一次发送、零自动重试；本地提交失败不得删除调用事实                                                                                                                                                       |
| 完整自动化 | 完整数据、桌面、类型、规范、格式、架构、敏感信息、正式构建、Rust 和正式浏览器旅程全部退出 0                                                                                                                                                                        |
| 人工边界   | 未执行的真实供应商、真实安装、系统级 200% 缩放、触摸与真人视觉明确披露，不得由模拟测试替代                                                                                                                                                                         |
| 唯一来源   | 候选来自一个干净提交；构建期间提交、索引、源码指纹和正式前端指纹不得变化                                                                                                                                                                                           |
| 安装包     | 本地 Tauri 候选名为 `墨影 InkShadow_0.2.16_x64-setup.exe`，GitHub 公开附件名为 `InkShadow_0.2.16_x64-setup.exe`；两者字节数和 SHA-256 必须一致。ProductName、FileDescription、ProductVersion 与 FileVersion 准确，载荷为 AMD64；未签名时 Authenticode 为 NotSigned |
| 五个附件   | 安装包、来源清单、四行 SHA-256、完整验证报告和正式发布说明来自同一提交；名称、大小和摘要在回下载后再次核对                                                                                                                                                         |
| 远端发布   | `main`、带说明标签 `v0.2.16`、Release 和来源清单指向同一提交；远端持续集成全部通过后才创建预发行                                                                                                                                                                   |

0.2.16 的实际命令、统计、未执行边界和发布事实记录在 [`2026-08-30-V0216-RELEASE.md`](2026-08-30-V0216-RELEASE.md) 及同名历史发布附件中。本节不得作为 0.2.17 的发布授权或当前运行证据。

## 0.2.15 未签名预发行门禁（历史）

| 门禁       | 准入要求                                                                                                                                                                      |
| ---------- | ----------------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| 生产缺陷   | 开书调度、写作技能、章节安全、版本恢复、规划、导出、旧数据兼容、连接恢复、向量分流和检查页缺陷必须绑定修复前后回归                                                            |
| 数据安全   | 正文、历史版本、灵感、设定和历史生成结果摘要保持不变；连续升级、备份与恢复必须覆盖当前全部用户表                                                                              |
| 完整自动化 | 完整数据、桌面、类型、规范、格式、架构、敏感信息、正式构建、Rust 和正式浏览器旅程全部退出 0                                                                                   |
| 人工边界   | 未执行的真实供应商、真实安装、系统级 200% 缩放、触摸与真人视觉必须明确披露，不得由模拟测试替代                                                                                |
| 唯一来源   | 候选必须来自一个干净提交；构建期间提交、索引、源码指纹和正式前端指纹不得变化                                                                                                  |
| 安装包     | 文件名为 `墨影 InkShadow_0.2.15_x64-setup.exe`；实际 ProductName、FileDescription、ProductVersion 与 FileVersion 均准确，应用载荷为 AMD64；未签名时 Authenticode 为 NotSigned |
| 五个附件   | 安装包、来源清单、四行 SHA-256、完整验证报告和正式发布说明来自同一提交，名称、大小和摘要必须在回下载后再次核对                                                                |
| 远端发布   | `main`、带说明标签 `v0.2.15`、Release 和来源清单指向同一提交；远端持续集成全部通过后才创建预发行                                                                              |

本轮实际命令、统计、未执行边界和发布事实记录在 [`2026-08-29-V0215-RELEASE.md`](2026-08-29-V0215-RELEASE.md) 以及同名发布附件中。未知的提交、摘要和链接不得预填。

## 0.2.14 未签名预发行门禁（历史）

| 门禁                 | 历史记录状态                         | 历史证据与当时准入要求                                                                                                                                                                            |
| -------------------- | ------------------------------------ | ------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| 报告与证据边界       | 已审计                               | 三份报告原件及 Q01–Q11、P02–P10、R05–R14 原始材料未取得，摘要不得冒充现场复现                                                                                                                     |
| R13-F2-PRO           | 自动化通过                           | 修复前 7／8、修复后 8／8；相关合同已经进入桌面 2,554 项全量。真实安装创建与恢复仍待人工                                                                                                           |
| R13-F3-DEDUP         | 聚焦自动化通过                       | Data `0079`／Tauri `82` 建立不可变多证据，Data `0081`／Tauri `84` 保持精确 UTF-16 核对并消除长正文重复扫描；故事核心 148／148、迁移恢复 52／52、原生迁移 18／18。最终全量候选链待重跑             |
| R13-V1-SELECTION     | 自动化通过／人工阻断                 | Data `0080`／Tauri `83` 已覆盖四项准确动作、历史空值、新旧混合恢复与重启；仍须真人鼠标、键盘和真实安装窗口完成改写、润色、扩写、缩写的真实调用闭环                                                |
| R13-V2／O1／O2       | 自动化通过／人工待验                 | 导航、中文展示、重启与账本合同已进入桌面及全仓测试；最终发布端到端、真人视觉、触摸、系统缩放、原生关闭和真实断网待验                                                                              |
| 界面合同             | 当前通过                             | `.ink-prose` 合同曾被误删；恢复后 UI 6 个文件、42／42 通过                                                                                                                                        |
| 数据、桌面和全仓测试 | 第二轮稳定化待同源重跑               | 第二次远端质量作业发现说明异步等待竞态和完整迁移直接写 Windows 临时文件的同步慢盘尾延迟；修复后四组 124／124、重开连续 10／10、桌面完整 2,554 通过、1 跳过；生产逻辑、20 秒上限和数据断言均未放宽 |
| 静态与原生门禁       | 上一同源提交通过／待重跑             | 当前工作区的全仓类型、规范和格式检查通过；39 项脚本、敏感信息、137 项许可、20 项边界、桌面发布配置及 Rust 207 项通过、1 项忽略均绑定 `400120f`，新候选必须重跑                                    |
| 桌面正式构建         | 上一同源提交通过／待重跑             | `400120f` 构建退出码 0：60 个文件、7,337,824／7,340,032 字节，余量 2,208 字节；最大异步分块 494,804／512,000 字节；新候选必须重跑                                                                 |
| 生产浏览器端到端     | 上一同源提交通过／待重跑             | `400120f` 的 `pnpm test:e2e` 为 26／26；新候选必须重跑，自动化等效 150%／200% 不替代真人和真实 Windows 系统缩放                                                                                   |
| 完整发布检查         | 上一同源提交通过／待重跑             | 首次失败证据保留；`400120f` 的完整 `release:check` 退出 0，新候选必须从头重跑                                                                                                                     |
| 正式发布端到端       | 上一同源提交通过／待重跑             | `400120f` 独立运行与候选链内均为 26／26；新的最终提交必须再次从同一提交重跑                                                                                                                       |
| 安装与人工验证       | 未执行并披露                         | 覆盖升级、启动、卸载、重装、外部打开、真实模型、真实系统 200% 缩放、触摸和真人视觉不写成通过                                                                                                      |
| 唯一提交与候选链     | 第三候选待形成                       | `400120f643f9fccfe929560a39e79f263a384b1d` 本地候选退出 0并已推送，但第二次远端运行 `33107276512` 失败关闭；必须从包含两项测试稳定化修复的新唯一提交重新完成本地候选链和三项远端门禁              |
| 五个发布附件         | 待新提交整体刷新                     | `400120f` 安装包、来源清单和摘要不能冒充新的发布源；新候选链通过后必须重建五项附件并重新核对版本、未签名状态、大小、摘要和来源                                                                    |
| 推送、标签和 Release | 中间两次发布曾停止；后续已发布并冻结 | 本表所述两次中间候选当时均停止发布；后续 `v0.2.14` 已从最终来源提交形成标签和未签名预发行，其标签、Release、安装包及附件现均不可移动、覆盖或复用。精确最终事实以冻结来源清单和公开 Release 为准   |

> [!IMPORTANT]
> [!NOTE]
> 以下为 0.2.13 历史候选门禁。其安装包与五项附件已经冻结，不替代上方 0.2.14 当前准入。

## 0.2.13 阻断缺陷修复候选门禁

当前结论：**F1 至 F6 的源码修复和分项回归已有当前证据，但完整发布准入尚未闭合。非当前不可变分支只允许显示已验证稳定正文，页面必须只读并阻断全部写入；任何最终门禁失败都不得生成候选包。即使候选包形成，本轮仍禁止创建标签或 GitHub Release。** 精确项目见 [`2026-08-26-V0213-BLOCKERS-REMEDIATION.md`](2026-08-26-V0213-BLOCKERS-REMEDIATION.md)。

| 门禁                      | 当前状态         | 当前证据或准入要求                                                                                                             |
| ------------------------- | ---------------- | ------------------------------------------------------------------------------------------------------------------------------ |
| 报告与现场证据审计        | 已完成边界登记   | 两份报告原文、截图 M01–M10／N01–N24、现场数据库、完整支持编号、诊断包、调用栈和供应商响应未取得；不得冒充现场复现              |
| F1 权威正文与版本保护     | 聚焦通过         | 当前父链不安全继续停写；非当前不可变分支保留、稳定正文可见但全页只读；较早恢复稿由作者显式决定。编辑器与真实文件 SQLite 73／73 |
| F2／F4／F5 设置与证据交互 | 聚焦通过         | 桌面相关组合 99／99；设置锚点 Chromium 6／6；证据交互重复运行 2／2；真实凭据管理器、真人视觉和真实触摸未执行                   |
| F3 重复待确认设定         | 聚焦通过         | 故事设定 20／20；桌面存储、整理与设定页 67／67；历史行不删除，作者决定不覆盖                                                   |
| F6 导出合同               | 聚焦通过         | 导入导出 96／96、桌面入口 24／24；空章节可导出，私密项目包在写字节前拒绝；原生保存和外部打开未执行                             |
| 数据、升级与备份恢复      | 当前分项通过     | 数据层 478／478；迁移与维护聚焦 62／62；桌面恢复与备份聚焦 44／44；本轮无数据库迁移                                            |
| 原生层与静态门禁          | 当前分项通过     | 原生 207 通过、0 失败、1 忽略；39 项脚本、137 项许可、20 项边界、敏感信息与发布配置检查通过                                    |
| 桌面端完整测试            | 当前通过         | 293 个文件、2,508 通过、0 失败、1 跳过                                                                                         |
| 全仓测试、类型与正式构建  | 当前通过         | 全仓 3,889 通过、65 跳过；类型与构建 20／21 个有脚本的工作区通过；载荷修复后正式目录 7,339,928／7,340,032 字节                 |
| 正式网页端到端            | 当前通过         | 首轮 24／25；准确项目包说明修复后定向 4／4，最终完整复跑 25／25                                                                |
| 唯一候选提交与完整候选链  | 首轮失败，待重跑 | 首轮在正式载荷门禁停止并且未打包；不得提高预算或沿用候选前构建结果，须从新的唯一干净提交从头重跑                               |
| 五个本地候选附件          | 尚未形成         | 首轮没有安装包；完整候选链通过后才保留安装包、来源清单、四行校验文件、完整中文验证报告和正式中文候选说明                       |
| 可再生资源清理            | 待本轮最终补录   | 只清理依赖、构建输出和测试缓存；保留 0.2.13 候选附件、v0.2.12 及更早历史证据和持久化演练数据库                                 |
| 标签与 GitHub Release     | 禁止执行         | 等待 0.2.13 真实安装人工复测结果及用户单独发布指令；不得覆盖 v0.2.12                                                           |

### 当前不构成发布通过的项目

- 没有真实供应商调用、真实安装程序人工复测、真实 Windows 凭据管理器、真实系统百分之二百缩放、真实触摸或真人视觉证据。
- 没有原生保存对话框、外部 Word／EPUB／PDF 打开和安装版备份恢复人工记录。
- 没有唯一候选提交、来源指纹、安装包大小、SHA-256、内部产品／文件版本、架构和签名终态；未知值不得预填。
- 本轮没有新增数据库迁移，也没有修改已发布迁移或校验值。

## 0.2.12 最终本地门禁完成（历史发布前快照）

历史时点结论：**报告审计、根因修复、数据安全、全仓测试、类型、规范、格式、正式构建、原生层和正式网页端到端均已取得当时源码快照通过证据；候选、推送、标签和 Release 在该时点尚未写成完成。** v0.2.12 后续已经发布并固定解析到 `8d20dfbbdac1eaecdde046714ba257257cd68ace`；本节保留当时门禁，不替代当前 0.2.14 结果。精确项目见 [`2026-08-25-V0212-RETEST-RELEASE.md`](2026-08-25-V0212-RETEST-RELEASE.md)。

| 门禁                                | 当前状态   | 当前证据或准入要求                                                                                          |
| ----------------------------------- | ---------- | ----------------------------------------------------------------------------------------------------------- |
| 报告、证据与冲突审计                | 已完成     | 缺失材料明确写“未取得”；P2-1 两轮结论不互相替代                                                             |
| 缺陷回归与根因修复                  | 已完成     | 连接状态、证据交互、任务身份、设置深链、四种选区操作和测试作品合同均有聚焦回归                              |
| 数据层与连续升级                    | 通过       | 数据层 81 个文件、478 项；连续升级 2 个文件、2 项；迁移回执上限 81                                          |
| 全仓与 Desktop 测试                 | 通过       | 全仓 509 个文件通过、16 个文件跳过，3,870 项通过、65 项跳过；Desktop 292 个文件、2,495 项通过、1 项跳过     |
| 类型、规范、格式和静态门禁          | 通过       | 类型 20／21；规范、格式、39 项脚本、敏感信息、137 项许可、20 项边界和发布配置均通过                         |
| 正式构建                            | 通过       | 20／21 个工作区；Desktop 7,335,567／7,340,032 字节，2,306 个模块、27 个分块，最大分块 489,186／512,000 字节 |
| 原生层                              | 通过       | 格式和严格静态检查通过；207 项通过、0 项失败、1 项忽略                                                      |
| 正式网页端到端                      | 通过       | 首轮 19／21；两条定位分别 1／1；最终 21／21；浏览器下载解析另有 1／1                                        |
| 外部与人工验证                      | 未执行     | 真实供应商、真人视觉、真实触摸设备、原生保存对话框、外部打开和真实百分之二百系统缩放不得写成通过            |
| 应用内浏览器                        | 未执行     | 两次在页面打开前被 `helper_unknown_error` 阻断，未取得页面或截图证据                                        |
| 唯一候选提交与五个附件              | 当时待执行 | 当时必须从一个干净提交重跑 `pnpm release:candidate:unsigned`，不得预填提交、大小或 SHA-256                  |
| 推送、标签、GitHub Release 与回下载 | 当时待执行 | 当时只有候选链和远端持续集成通过后才能执行；后续发布事实不由本表补写                                        |

首次失败没有被删除：全仓测试曾有 2 项失败，正式构建曾超预算 3,974 字节，Rust 曾因迁移期望 80 落后于真实上限 81 而出现 1 项失败，正式网页端到端曾有 2 条旧定位失败；各项修复和最终复跑详见专项记录与 `TEST_RESULTS.md`。

该章节是发布提交形成前的源码快照。实际发布提交、来源指纹、标签、附件和 Release 结果应写入发布附件中的中文验证报告，不通过发布后的额外源码提交改变标签与远端 `main` 的同源关系。

## 0.2.11 阻断缺陷与界面系统修复候选门禁

当前结论：**证据审计、根因修复、源码门禁、原生门禁与提交前生产网页旅程已通过；唯一干净提交上的正式候选链、安装包、清理和人工复测尚待执行。D1、D5 或任何数据安全门禁失败时不得生成候选包，更不得建议发布。** 精确项目见
[`2026-08-24-V0211-BLOCKERS-UI-REMEDIATION.md`](2026-08-24-V0211-BLOCKERS-UI-REMEDIATION.md)。

| 门禁                  | 当前状态       | 准入要求                                                            |
| --------------------- | -------------- | ------------------------------------------------------------------- |
| 报告与证据审计        | 已完成只读提取 | 缺失材料明确写“未取得”，原始证据不修改、不提交                      |
| 修复前回归与根因修复  | 已完成         | 自动化缺陷保留红灯，根因和当前绿灯绑定同一源码                      |
| 数据保护与完整验证    | 源码门禁通过   | 干净提交候选链仍须重跑完整源码、原生和正式端到端门禁                |
| 0.2.11 人工复测包     | 待唯一提交     | 仅从唯一干净提交构建，核对内部版本、x64、未签名状态、大小与 SHA-256 |
| 标签与 GitHub Release | 禁止执行       | 等待真实旧库覆盖升级、真实远程供应商人工复测和单独发布指令          |

## 0.2.10 两轮真实安装复测候选门禁（历史）

当前结论：**源码修复、聚焦回归以及提交前的数据、桌面、类型、规范、格式、构建、原生和静态安全门禁已经通过；正式网页端到端、完整候选链、候选提交、未签名安装包与清理仍须从唯一干净提交完成。真实安装人工复测前不得创建标签或 Release。** 精确项目见
[`2026-08-24-V0210-ROUND2-REMEDIATION.md`](2026-08-24-V0210-ROUND2-REMEDIATION.md)。

| 门禁                  | 当前状态       | 准入要求                                                                                         |
| --------------------- | -------------- | ------------------------------------------------------------------------------------------------ |
| 报告与证据审计        | 已完成只读提取 | 缺失材料明确写“未取得”，原证据不提交                                                             |
| 修复前回归与根因修复  | 已完成         | 自动化缺陷已有失败证据、源码根因和当前工作树通过结果                                             |
| 全量验证              | 提交前门禁通过 | 数据、桌面、类型、规范、格式、构建、原生和静态安全已通过；正式网页旅程及完整候选链待干净提交重跑 |
| 0.2.10 人工复测包     | 待唯一提交     | 必须来自同一干净提交，内部版本和哈希复核通过，明确未签名                                         |
| 标签与 GitHub Release | 禁止执行       | 等待 v0.2.10 真实安装人工复测结果和单独发布指令                                                  |

## 0.2.9 真实接口报告修复候选门禁（历史形成阶段）

候选形成阶段结论：**报告缺陷聚焦回归、数据层和桌面端当时全量、静态检查、开发服务器网页旅程、正式构建及原生层最终门禁已有通过证据；完整干净提交候选链、正式发布版网页端到端、未签名安装包、真实安装和发布在该阶段仍待执行。** 本节编号对应 v0.2.8 报告，不改写下方历史版本结论；标为“独立安全审计”的项目不是报告新增缺陷。`v0.2.9` 后续已从提交 `54d9647031bb97b4fc9f021d3b1acca7f6d25c47` 发布，本表原有数字仍只描述候选形成阶段。
