...
> 当前发布目标：`0.2.14`。0.2.13 五项本地候选附件和 v0.2.12 及更早公开发布保持不可变。0.2.14 已取得完整本地候选链证据；最新用户指令授权在最终同源重建和远端持续集成通过后发布未签名预发行。
...
| 数据、桌面和全仓测试 | 数据稳定化待同源重跑     | 首轮远端完整权威链恢复超过 Windows 文件型 SQLite 通用 15 秒基础设施上限并引发共享临时文件级联；专项 30 秒、独立路径和关闭后清理修复后，聚焦 3／3、维护 51／51、数据 481／481通过；功能断言未删减 |
| 静态与原生门禁       | 当前通过                 | 类型 20／21、规范、格式、39 项脚本、敏感信息、137 项许可、20 项边界、桌面发布配置通过；Rust 207 通过、1 忽略                                                                                     |
| 桌面正式构建         | 当前通过                 | 退出码 0；60 个文件、7,337,824／7,340,032 字节，余量 2,208 字节；最大异步分块 494,804／512,000 字节；主 CSS 约 127.36 KB／128 KB                                                                 |
| 生产浏览器端到端     | 当前通过                 | `pnpm test:e2e` 为 26／26；自动化等效 150%／200% 不替代真人和真实 Windows 系统缩放                                                                                                               |
| 完整发布检查         | 当前通过                 | 首次失败证据保留；恢复 `.ink-prose` 后，完整 `release:check` 已在干净候选链中退出 0                                                                                                              |
| 正式发布端到端       | 当前通过                 | 独立运行与候选链内均为 26／26；最终发布文案提交形成后必须再次从同一提交重跑                                                                                                                      |
| 安装与人工验证       | 未执行并披露             | 覆盖升级、启动、卸载、重装、外部打开、真实模型、真实系统 200% 缩放、触摸和真人视觉不写成通过                                                                                                     |
| 唯一提交与候选链     | 新候选待形成             | `bd7e3cb96c970eb7c0bba53944401ee8dbcafc04` 本地候选退出 0并已推送，但首轮远端运行 `33101514463` 失败关闭；必须从包含测试稳定化修复的新唯一提交重新完成本地候选链和三项远端门禁                   |
| 五个发布附件         | 待新提交整体刷新         | 旧 `bd7e3cb` 安装包、来源清单和摘要不能冒充新的发布源；新候选链通过后必须重建五项附件并重新核对版本、未签名状态、大小、摘要和来源                                                                |
| 推送、标签和 Release | 推送已完成一次，发布停止 | `main` 当前含首轮候选；没有创建 `v0.2.14` 标签或 Release。只有新提交的本地候选和三项远端持续集成全部成功后，才创建带说明标签和未签名预发行                                                       |
