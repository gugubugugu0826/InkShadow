...
> [!IMPORTANT]
> 当前应用清单已升至 `0.2.6`，最新公开版本仍为 `v0.2.5`。v0.2.6 当前工作树的全量自动化、
> 原生层检查、正式网页构建、体积预算和浏览器端到端回归已经通过；隔离 Windows Tauri 真机、
> 真实模型、干净提交、推送、打包和发布仍待完成。`v0.2.5` 继续绑定唯一来源提交
> `5b3e212cafde10cd75fa87b7b74bfdfff9347a3d`，不得移动或复用；更早版本说明按原日期保留。

> 更新日期：2026-08-21  
> 当前源码版本：`0.2.6`；最新已发布工程预览版本：`0.2.5`  
> 当前结论：**当前工作树自动化已通过；隔离真机与真实模型待验证；提交、推送、打包和发布未完成**
...
> 不建议用于真实 Provider 的敏感正文处理。`v0.2.5` 的自动化证据来自 fake/mock、真实临时 SQLite、
> Rust/Tauri 单元集成与 production Chromium；虽已生成并公开新的未签名 Windows 安装包，Provider
> live 与真实 Tauri 人工复测仍未运行，不能把源码自动化写成真实平台验证。
...
v0.2.6 当前结果来自尚未提交的共享工作树，不是干净发布候选。检查时远端 `main` 仍为
`6fd32239b30de9907a0a83b5ce3680e1a3830377`，与本地基线领先/落后均为 0；当前修复尚未推送。

| 门禁                    | 状态                                 | 当前证据与下一动作                                                                                                                        |
| ----------------------- | ------------------------------------ | ----------------------------------------------------------------------------------------------------------------------------------------- |
| 应用清单版本            | 通过                                 | 根清单、桌面清单、原生清单、桌面配置和锁文件已统一为 `0.2.6`；最新公开版本仍为 `v0.2.5`                                                   |
| D1 本地数据与重载       | 自动化通过，真机待验证               | 先进先出连接队列、有界等待、会话失效和重载恢复已覆盖；仍需真机连续重载、进程重启、跨章节和跨作品                                          |
| D2 使用 AI 建议草稿     | 自动化通过，真机待验证               | 核心接受原子提交不等待可重建派生；仍需真机锁竞争、重复点击、强制结束和重启                                                                |
| D3 开书三个方案         | 自动化通过，真机待验证               | 三槽独立结算、部分成功保留、结果待核对不重发；仍需真机三槽完整对账                                                                        |
| D4 自动备份             | 前端与原生自动化通过，真机恢复待验证 | 第 2 版状态机、独立原生快照、成功项启动复验和旧健康备份回退已覆盖；仍需真实桌面创建与独立恢复                                             |
| D5 能力验证账本         | 自动化通过，真实模型待验证           | 新增独立任务、原生发送回执和一发一条调用记录；仍需“披露→确认→一次调用→一条记录”真机对账                                                   |
| 数据迁移                | 通过                                 | 新增前向迁移 `0071_model_capability_probe_invocation_ledger.sql`，桌面原生内部版本为 `74`；未改已发布迁移或摘要                           |
| 凭据命名空间            | 自动化通过，隔离进程待验证           | 从 Tauri 应用标识派生；正式标识 `com.inkshadow.desktop` 保持原值；模型、设备、项目、云端和更新凭据均使用隔离命名空间，不迁移、复制或删除  |
| 全工作区测试            | 通过                                 | `pnpm test` 退出码 0；472 个测试文件通过、16 个测试文件跳过；3,418 项通过、65 项跳过；桌面端 266/2,095+1 跳过，网页端 4/33，数据层 70/445 |
| 七项静态门禁            | 通过                                 | 格式 21.38 秒；敏感信息扫描 0；许可证 137；包边界 20；桌面发布配置 17/17；类型检查 20/21；代码规范 0 条警告                               |
| 原生层                  | 通过                                 | 格式通过；严格静态检查首次 1 条测试初始化警告，修复后通过；测试 190 项通过、1 项忽略、0 项失败                                            |
| 正式网页构建与预算      | 通过                                 | 2,286 个模块；58 个文件 / 7,068,735 字节；预算计数 7,067,935 / 7,340,032；入口、异步块、样式和工作线程均在既有上限内                      |
| 浏览器端到端            | 通过                                 | 13/17（旧英文定位）→ 16/17（一个遗漏）→ 17/17；最终 26.1 秒，普通界面定位均为中文                                                         |
| 隔离 Windows Tauri 真机 | 未运行                               | 必须在独立应用标识和独立数据库下复跑 D1–D5；浏览器与临时数据库不能替代                                                                    |
| 真实模型与费用对账      | 未运行                               | 先声明调用次数、词元和费用上限，再执行三槽开书与一次固定能力验证；必须逐条对齐本地调用账本                                                |
| 干净唯一提交            | 未运行                               | 正式发布候选命令已在脏工作树清洁门按预期拒绝；先冻结并提交，再从唯一提交完整重跑                                                          |
