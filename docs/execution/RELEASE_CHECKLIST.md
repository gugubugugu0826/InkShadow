...
| R13-F3-DEDUP         | 自动化通过           | Data `0079`／Tauri `82`；故事核心 148／148、数据层 481／481，覆盖完整迁移、连续升级、旧备份回填、当前恢复和事实多证据。现场数据库未取得            |
...
| 唯一提交与候选链     | 本地候选通过         | `b4ec54feb51dc651201daa14e635b261bd9f592c` 的完整未签名候选链退出 0；正式文案提交形成后重新取得最终同源证据                                        |
