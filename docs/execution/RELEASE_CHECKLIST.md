...
> [!IMPORTANT]
> 当前应用清单已升至 `0.2.6`，最新公开版本仍为 `v0.2.5`。D1–D5 已在提交 `722e67e`
> 的隔离 Windows Tauri 程序完成聚焦复测；`722e67e` 是当前修复源码提交，包含本报告的干净候选
> 提交仍待创建，其全量验证、真实外部模型、推送、打包和发布仍待完成。`v0.2.5` 继续绑定唯一来源提交
> `5b3e212cafde10cd75fa87b7b74bfdfff9347a3d`，不得移动或复用；更早版本说明按原日期保留。

> 更新日期：2026-08-21  
> 当前源码版本：`0.2.6`；最新已发布工程预览版本：`0.2.5`  
> 当前结论：**隔离真机聚焦复测完成；最终全量候选与真实外部模型待运行；推送、打包和发布未完成**
...
> 不建议用于真实模型服务的敏感正文处理。`v0.2.5` 的自动化证据来自严格模拟服务、真实临时 SQLite、
> Rust/Tauri 单元集成与 production Chromium；虽已生成并公开新的未签名 Windows 安装包，真实模型服务
> 与真实 Tauri 人工复测仍未运行，不能把源码自动化写成真实平台验证。

v0.2.6 当前区只使用“通过”“进行中”“未运行”“外部条件阻塞”和“不适用”。历史区保留当时的
状态原文。代码存在不等于发布门禁通过；只有可复核的测试、构建、签名、安装或人工验收证据才能标记通过。

## v0.2.6 发布前清单（2026-08-21）

v0.2.6 源码修复链为 `374b3ab` → `4ffa1ba` → `722e67e`，不是最终发布候选。远端 `main`
仍为 `6fd32239b30de9907a0a83b5ce3680e1a3830377`；本地领先 3 个提交且尚未推送。

| 门禁                    | 状态         | 当前证据与下一动作                                                                                                                                  |
| ----------------------- | ------------ | --------------------------------------------------------------------------------------------------------------------------------------------------- |
| 应用清单版本            | 通过         | 根清单、桌面清单、原生清单、桌面配置和锁文件已统一为 `0.2.6`；最新公开版本仍为 `v0.2.5`                                                             |
| D1 本地数据与重载       | 通过         | 隔离真机中 2 个作品、4 章连续重载 3 次、进程重启 2 次；数据库完整性正常、外键违规 0，逻辑快照不变                                                   |
| D2 使用 AI 建议草稿     | 通过         | 隔离真机中正常、重复点击、事务故障、派生故障、锁等待后强停/重启均满足合同；主动取消尚未单独真机覆盖                                                 |
| D3 开书三个方案         | 通过         | 隔离真机中 1 槽成功、1 槽明确失败、1 槽结果待核对；重载与进程重启均为 0 次重发，成功槽保留 852 字                                                   |
| D4 自动备份             | 通过         | 非空作者数据真机生成 3,653,632 字节成功备份；冻结源库与全新恢复目录 172 张表逐表差异 0，两份昨日健康文件均保留；权限/磁盘/竞争/写入中强停真机未覆盖 |
| D5 能力验证账本         | 通过         | 回环真机取消为 0；确认后恰好 1 次发送、1 条成功模型调用记录和 1 份能力证据；真实外部模型仍为外部条件阻塞                                            |
| 数据迁移                | 通过         | 新增前向迁移 `0071_model_capability_probe_invocation_ledger.sql`，桌面原生内部版本为 `74`；未改已发布迁移或摘要                                     |
| 凭据命名空间            | 通过         | 隔离程序使用独立标识、数据和凭据命名空间；正式五类凭据、正式数据库和用户目录均未读取或修改                                                          |
| 全工作区测试            | 未运行       | `374b3ab` 曾为 472 个文件通过、16 个跳过；3,418 项通过、65 项跳过；不能替代包含本报告的候选全量                                                     |
| 七项静态门禁            | 未运行       | `374b3ab` 上全部通过；候选提交上的格式、敏感信息、许可证、边界、发布配置、类型和代码规范须完整重跑                                                  |
| 原生层                  | 未运行       | `4ffa1ba` 上格式/严格静态检查及 190 通过、1 忽略；当前修复源码 `722e67e` 只有回执差量 19/19                                                         |
| 正式网页构建与预算      | 未运行       | `374b3ab` 快照为 2,286 个模块、58 个文件 / 7,068,735 字节，预算计数 7,067,935 / 7,340,032；不得迁移为最终候选结果                                   |
| 浏览器端到端            | 未运行       | `374b3ab` 失败链为 13/17 → 16/17 → 17/17，最终 26.1 秒；包含本报告的候选提交创建后须重跑                                                            |
| 隔离 Windows Tauri 真机 | 通过         | `722e67e` 程序 25,886,208 字节、SHA-256 `2c0e1da5…f0a139`；不是安装包，也不替代最终候选                                                             |
| 回环调用与账本对账      | 通过         | 4 次：7/1、321/512、未知/未知、未知/未知词元；费用均未知；四条记录、尝试均 1、自动重试均 0                                                          |
| 真实外部模型与费用对账  | 外部条件阻塞 | 实际调用 0 次、输入/输出词元 0、费用 0；已声明上限 4 次、20,000 合计词元、人民币 2 元                                                               |
| 最终干净唯一提交        | 未运行       | 早期清洁门曾按预期拒绝；文档同步后须冻结最终提交，再从该提交完整重跑候选                                                                            |
| 推送与远端流水线        | 未运行       | 当前修复未推送；不得引用 v0.2.5 远端结果作为 v0.2.6 结果                                                                                            |
| 来源指纹与发布清单      | 未运行       | 清洁门通过后生成；当前不填写临时值                                                                                                                  |
| Windows 未签名安装包    | 未运行       | 必须来自同一干净提交；记录文件名、字节、产品版本和 SHA-256                                                                                          |
| 标签、公开发布与回下载  | 未运行       | 不移动旧标签；发布后重新下载每个附件并逐一核对文件名、字节和 SHA-256                                                                                |

### 当前失败链必须保留

- 首次全工作区运行有 2 项开书页测试超过默认 1 秒；只为这两项加入 15 秒终态等待后，聚焦
  2/2 通过，最终全工作区全部通过。
- 数据层结构化原生错误包装首次有 5 项失败；修正错误对象保留后，数据层 70 个文件 / 445 项通过。
- 原生层严格静态检查首次有 1 条测试初始化告警；等价修正后全绿。
- 浏览器端到端按 13/17 → 16/17 → 17/17 收口，最终 26.1 秒；不得只记录最终数字而删除失败轨迹。
- `374b3ab` 隔离程序首次启动发生原生主线程栈溢出；装箱大型异步命令后，`4ffa1ba` 不再溢出，
  且原生层 190 项通过、1 项忽略。
- `4ffa1ba` 自动备份已生成完整文件，却因原生蛇形回执与前端驼峰端口不匹配而保守显示“结果待核对”；
  `722e67e` 修正字段后首启直接成功，差量 19/19，并完成独立恢复。

### 隔离真机证据边界

- D4 首轮先以空作品库完成清单修订 4：成功备份为 3,608,576 字节，SHA-256 为
  `7112e8aaee5c555e4af899cb369a8bafd3f7f6417b04067ecc3797cf6e40d227`，并完成 172 表独立恢复；
  该证据只作为文件级创建与恢复的首次通过。
- 随后通过正式界面接受 1 份就绪 AI 建议草稿，源库成为 4 个作品、6 章、7 个不可变版本、
  1 份 AI 建议草稿、1 个后台任务、4 条模型调用记录、2 个开书旅程和 9 轮问答，模型调用增量为 0。
  今日清单修订 5 成功，文件为 3,653,632 字节，SHA-256
  `c98594980d258d0d11469a2d102f2f42575d073d854d3209719ee5aa02b96f04`。
- 首次比较使用仍在运行的源库，设置页在备份完成后初始化了 1 行可重建的
  `story_memory_policies`，因此与备份时点相差 1 行并按合同失败；失败证据保留在
  `.tmp/v026-tauri-regression/evidence/backup-restores/d4-nonempty-due-restore/verification.json`。
  按“启动前冻结源库 → 今日备份 → 全新恢复目录”复跑后，172 张表逐表差异 0，源/恢复完整性正常、
  外键违规均为 0，上述八类作者数据非空且一致；通过证据为
  `.tmp/v026-tauri-regression/evidence/backup-restores/d4-nonempty-due-restore-source-freeze/verification.json`
  和 `.tmp/v026-tauri-regression/evidence/backup-comparisons/d4-nonempty-final-contract.json`。
- 旧创建中且目标缺失的条目已恢复为“结果待核对”，较旧健康文件 3,637,248 字节及摘要保持不变；
  没有覆盖重试或删除健康备份。
- 今日成功备份没有删除两份昨日健康文件；它们仍为 3,637,248 字节 / `0a2d8a…c6ef` 与
  3,682,304 字节 / `298b85…de5`。权限拒绝、磁盘写入失败、目标竞争和写入中强制结束仍只有
  自动化证据，未提升为最终安装包真机通过。

完整缺陷时序见
[`2026-08-21-V026-REAL-DEVICE-DEFECT-REMEDIATION.md`](2026-08-21-V026-REAL-DEVICE-DEFECT-REMEDIATION.md)，
精确自动化结果见 [`TEST_RESULTS.md`](TEST_RESULTS.md)。
...
`v0.2.5` 已从唯一干净来源提交 `5b3e212cafde10cd75fa87b7b74bfdfff9347a3d` 完成源码门禁、
unsigned candidate、远端 CI、annotated tag、三个公开附件与回下载复核，并作为 Pre-release 发布。
真实 Provider 与 Windows Tauri 第二阶段复测保持 `BLOCKED_EXTERNAL`；`v0.2.4` 与 `v0.2.5` 的既有
标签和附件均不能复用、移动或静默替换。
...
**当前结论：`v0.2.5` 已公开为不可静默替换的最新 Pre-release；`v0.2.3` 的隐式云调用安全公告继续有效。它们都不是 Beta、GA 或商业正式版；上述外部验证完成前不得扩大发布结论。**
