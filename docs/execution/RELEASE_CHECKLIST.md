...
> [!CAUTION]
> 当前工作树已进入 `0.2.7`，但尚未形成干净候选。最新公开版本仍是不可移动的 `v0.2.6` 未签名工程预发布。下方原有 `v0.2.6` 门禁、提交、标签、哈希和回下载记录均为历史事实，不能作为 `v0.2.7` 通过证据。
...
| 发布版浏览器端到端         | 未运行       | `pnpm test:e2e:release` 通过并保存本轮证据                                             |
...
| 未签名候选打包             | 未运行       | `pnpm release:candidate:unsigned` 与 `pnpm release:verify:unsigned` 通过               |
