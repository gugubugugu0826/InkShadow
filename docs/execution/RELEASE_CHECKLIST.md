...
| 数据、桌面和全仓测试 | 当前通过             | 故事核心 148；数据 481；桌面 296 个文件、2,554 通过、1 跳过；全仓 514 个文件通过、16 个文件跳过，3,945 通过、65 跳过、0 失败                                                          |
...
| 唯一提交与候选链     | 最终同源证据待重跑   | `b4ec54feb51dc651201daa14e635b261bd9f592c` 的旧候选链退出 0；最终文案提交首次重跑发现并修复长正文证据性能阻断，必须从含 `0081` 的新唯一提交重新完整退出 0                             |
| 五个发布附件         | 本地已形成           | 五项附件已核对版本、未签名状态、大小、摘要和来源；最终提交重建后必须整体刷新，旧候选不得冒充新来源                                                                                    |
| 推送、标签和 Release | 已获授权，条件执行   | 最新用户指令授权发布未签名预发行；只有最终候选链和同一提交三项远端持续集成全部成功后，才创建带说明标签和 Release                                                                      |
