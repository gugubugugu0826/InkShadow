...
> 当前发布目标：`0.2.14`。0.2.13 五项本地候选附件和 v0.2.12 及更早公开发布保持不可变。当前尚未形成 0.2.14 唯一候选提交、安装包、标签或 Release。
...
| 完整发布检查         | 未取得终局全绿       | 首次 `release:check` 在 UI 一项合同失败后停止；恢复 `.ink-prose` 后 UI、全仓测试和桌面构建分别通过，未取得修复后单次完整退出 0 的记录              |
| 正式发布端到端       | 脏工作区停止         | `pnpm test:e2e:release` 在捕获来源前按设计拒绝未提交工作区，0 条旅程启动；须在唯一干净提交后重跑                                                   |
| 安装与人工验证       | 未执行               | 覆盖升级、启动、卸载、重装、外部打开、真实模型、真实系统 200% 缩放、触摸和真人视觉                                                                 |
| 唯一提交与候选链     | 待执行               | 从干净唯一提交完整重跑发布端到端并运行 `pnpm release:candidate:unsigned`                                                                           |
| 五个发布附件         | 尚未形成             | 不预填提交、大小、摘要或签名结论；事实产生后写入验证报告与发布说明                                                                                 |
| 推送、标签和 Release | 禁止执行             | 干净来源候选链和必需人工门禁全部通过后才可执行                                                                                                     |
