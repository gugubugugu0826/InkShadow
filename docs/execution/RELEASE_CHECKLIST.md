...
> 当前发布目标：`0.2.16`。v0.2.15 及更早标签、Release、安装包、附件和来源证据保持不可变。最新用户指令授权在新的最终同源候选链和远端持续集成全部通过后发布未签名预发行。
...
本轮实际命令、统计、未执行边界和发布事实记录在 [`2026-08-30-V0216-RELEASE.md`](2026-08-30-V0216-RELEASE.md) 及同名发布附件中。未知提交、总数、摘要和链接不得预填。
