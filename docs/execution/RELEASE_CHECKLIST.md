...
> [!CAUTION]
> 当前工作树已进入 `0.2.7`。初始干净提交 `95904b39a0c2e6c21c30beb0bf02c891dbd798d4` 的候选端到端实际为 10／17 并按失败关闭；修正后的最终候选仍待从新提交完整重跑。最新公开版本仍是不可移动的 `v0.2.6` 未签名工程预发布。下方原有 `v0.2.6` 门禁、提交、标签、哈希和回下载记录均为历史事实，不能作为 `v0.2.7` 通过证据。

## v0.2.7 当前发布门禁

当前结论：**不允许发布为已完成；只可继续准备未签名工程预发布。**

| 门禁                       | 当前状态      | 当前证据或完成条件                                                                     |
| -------------------------- | ------------- | -------------------------------------------------------------------------------------- |
| 应用清单版本               | 本工作树通过  | 根清单、桌面清单、原生清单、桌面配置和锁文件均为 `0.2.7`                               |
| 直接模式一键远程生成       | 需独立授权    | 一次点击远程生成仍须用户另行明确授权；未执行真实模型调用，不得标为完成                 |
| 故事资料用户治理           | 本工作树通过  | 普通资料修改、删除后恢复、同文合并，以及结构化资料禁止明文修改和自动合并的完整测试通过 |
| 格式、类型与规范           | 本工作树通过  | `pnpm format:check`、`pnpm typecheck`、`pnpm lint` 均退出码为零                        |
| 正式构建                   | 本工作树通过  | `pnpm build` 退出码为零                                                                |
| 完整发布检查               | 候选首跑失败  | `ddd121b` 通过静态与前置工作区后，桌面端 1 项旧节点竞态失败；修正后须新提交完整重跑    |
| 全工作区测试               | 本工作树通过  | 480 个文件通过、16 个跳过；3,558 项通过、65 项跳过                                     |
| 桌面端测试                 | 本工作树通过  | 271 个文件通过；2,216 项通过、1 项跳过                                                 |
| 网页端测试                 | 本工作树通过  | 33/33 项通过                                                                           |
| 原生层                     | 本工作树通过  | 格式、严格静态检查和测试通过；194 项通过、1 项按条件忽略                               |
| 发布版浏览器端到端         | 工作树 17／17 | 初始干净提交补齐浏览器后为 10／17；修正后的发布网页构建 17／17，新提交仍须完整重跑     |
| 目标宽度、浅深色与真机交互 | 未完成        | 1440、1280、1024、800、浅色、深色、键盘和焦点仍需在最终安装程序完成真机矩阵            |
| Windows 系统百分之二百缩放 | 未运行        | 最终安装程序真机验证，无水平裁切或不可达操作                                           |
| 真实外部模型与费用         | 需独立授权    | 未执行真实模型或付费调用；须逐次记录真实服务、模型、回执、可见结果和费用               |
| 私密章节                   | 待最终验证    | 最终安装程序中验证外发前失败关闭，发送正文为零                                         |
| 备份恢复                   | 待最终验证    | 最终候选验证新迁移与全部用户创作表完整恢复，恢复前快照和失败保留原数据通过             |
| 干净唯一源码               | 未生成        | 清洁工作树的唯一提交，记录完整提交值和来源指纹                                         |
| 未签名候选打包             | 按失败停止    | 三次均在打包前停止；新提交的 `pnpm release:candidate:unsigned` 与验证必须完整通过      |
| 安装、升级、卸载真实操作   | 未运行        | 使用最终未签名安装程序验证安装、启动、升级、卸载、重装与迁移恢复                       |
| 远端持续集成               | 未运行        | 推送最终源码后，质量、Windows 原生层和云数据库任务全部成功                             |
| 推送、标签与公开预发布     | 未执行        | 远端 `main` 复核后推送；新建不可移动的 `v0.2.7` 带说明标签和工程预发布                 |
| 公开回下载                 | 未执行        | 全新目录下载恰好三个附件，文件名、大小、摘要及清单提交全部一致                         |

待回填：最终提交 `待生成`；来源指纹 `待生成`；网页制品指纹 `待生成`；安装程序、发布清单和校验文件的大小与 SHA-256 `待生成`；标签对象及解析值 `待生成`；持续集成运行编号 `待生成`；公开地址与回下载目录 `待生成`。
...
`v0.2.6` 已从唯一干净来源提交 `b744d042eeafdd9db586388d71e701b1d937f366` 完成源码门禁、
未签名候选、远端持续集成、带说明标签、恰好三个公开附件与回下载复核，并作为工程预发布发布。
真实外部模型与最终安装程序扩展真机矩阵仍为外部条件阻塞；`v0.2.4`、`v0.2.5` 与 `v0.2.6` 的既有
标签和附件均不能复用、移动或静默替换。
...
**当前结论：`v0.2.6` 已公开为不可静默替换的最新工程预发布；`v0.2.3` 的隐式云调用安全公告继续有效。既有版本都不是测试版、正式通用版或商业正式版；上述外部验证完成前不得扩大发布结论。**
