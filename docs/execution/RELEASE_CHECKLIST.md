...
> 当前发布目标：`0.2.14`。0.2.13 五项本地候选附件和 v0.2.12 及更早公开发布保持不可变。0.2.14 两次远端候选均在失败后停止发布；最新用户指令授权在新的最终同源重建和远端持续集成全部通过后发布未签名预发行。
...
| 推送、标签和 Release | 两次推送，发布均停止     | `main` 当前含第二次候选；没有创建 `v0.2.14` 标签或 Release。只有新提交的本地候选和三项远端持续集成全部成功后，才创建带说明标签和未签名预发行                                                      |
