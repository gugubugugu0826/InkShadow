...
> [!IMPORTANT]
> 当前源码快照应用清单为 `0.2.13`。首轮完整候选链因正式前端载荷 7,340,702／7,340,032 字节而失败关闭，没有生成安装包；载荷修复后的单独构建已低于上限，仍须从新的唯一干净提交从头重跑。已发布 v0.2.12 与更早候选、标签、Release、附件和来源证据保持不可变。0.2.13 真实安装人工复测及单独发布指令前禁止创建标签或 GitHub Release。
