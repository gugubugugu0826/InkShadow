...
> [!IMPORTANT]
> 当前源码快照应用清单为 `0.2.12`。最终本地源码、原生与正式网页端到端门禁已完成；唯一候选提交、远端 `main`、安装包、标签、GitHub Release 和回下载仍须由后续发布链确认。0.2.11 及更早候选、标签、Release、附件和来源证据保持不可变。

## 0.2.12 最终本地门禁完成

当前结论：**报告审计、根因修复、数据安全、全仓测试、类型、规范、格式、正式构建、原生层和正式网页端到端均已取得当前源码快照通过证据。发布准入的下一步是从唯一干净提交重跑完整未签名候选链；候选、推送、标签和 Release 尚未写成完成。** 精确项目见 [`2026-08-25-V0212-RETEST-RELEASE.md`](2026-08-25-V0212-RETEST-RELEASE.md)。
...
| 唯一候选提交与五个附件              | 待执行   | 必须从一个干净提交重跑 `pnpm release:candidate:unsigned`，不得预填提交、大小或 SHA-256                      |
| 推送、标签、GitHub Release 与回下载 | 待执行   | 只有候选链和远端持续集成通过后才能执行；未签名安装包必须作为预发行                                          |
