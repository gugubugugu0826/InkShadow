...
> [!IMPORTANT]
> 当前源码快照应用清单为 `0.2.13`。源码修复、桌面端完整测试、全仓测试、类型、规范、构建和正式网页端到端已有当前证据；唯一候选提交、完整未签名候选链、安装包和清理结果仍待本轮最终补录。已发布 v0.2.12 与更早候选、标签、Release、附件和来源证据保持不可变。0.2.13 真实安装人工复测及单独发布指令前禁止创建标签或 GitHub Release。
...
| 全仓测试、类型与正式构建  | 当前通过       | 全仓 3,889 通过、65 跳过；类型与构建 20／21 个有脚本的工作区通过；桌面策略体积 7,340,007／7,340,032 字节                       |
| 正式网页端到端            | 当前通过       | 首轮 24／25；准确项目包说明修复后定向 4／4，最终完整复跑 25／25                                                                |
| 唯一候选提交与完整候选链  | 待本轮最终补录 | 从一个干净提交执行 `pnpm release:candidate:unsigned`，记录提交、来源指纹、安装包内部版本、AMD64、签名、大小与 SHA-256          |
| 五个本地候选附件          | 待本轮最终补录 | 安装包、来源清单、四行校验文件、完整中文验证报告、正式中文候选说明；安装包未签名时标记“未签名工程预览”                         |
...
历史时点结论：**报告审计、根因修复、数据安全、全仓测试、类型、规范、格式、正式构建、原生层和正式网页端到端均已取得当时源码快照通过证据；候选、推送、标签和 Release 在该时点尚未写成完成。** v0.2.12 后续已经发布并固定解析到 `8d20dfbbdac1eaecdde046714ba257257cd68ace`；本节保留当时门禁，不替代当前 0.2.13 结果。精确项目见 [`2026-08-25-V0212-RETEST-RELEASE.md`](2026-08-25-V0212-RETEST-RELEASE.md)。
