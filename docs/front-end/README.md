...
> 2026-08-29 0.2.15 当前增量：开书多方案采用有序调度并提供单方案与同作品空白正文入口；写作技能区分已启用、实际采用与未采用原因，并提供个人技能管理；编辑器持续显示准确章节身份，大幅删除先保留恢复草稿；版本恢复、规划空状态、原生导出、待确认设定和普通界面中文均已接入生产入口。全部结果继续隔离，私密章节继续只在本机处理。完整门禁与真实安装状态见当前执行记录。
...
> 基于源码复核：2026-08-29  
> 当前发布目标：`0.2.15`；只有唯一干净提交通过全部门禁后才能形成未签名预发行；v0.2.14 及更早标签、Release、附件和来源证据保持不变  
