...
以上是 0.2.16 当前工作区的界面合同，不是发布或人工通过声明。F17 修复前后回归已经取得红绿证据；其余聚焦统计、完整门禁、真实安装、真实服务、真人交互和 Windows 系统级 200% 缩放仍须按当前提交实际记录。准确边界见 [0.2.16 修复与发布验证记录](../execution/2026-08-30-V0216-RELEASE.md)。
...
以上是 0.2.15 当时工作区的界面合同，不是 0.2.16 发布或人工通过声明。该版本的聚焦回归、根因和发布边界见 [0.2.15 修复与发布验证记录](../execution/2026-08-29-V0215-RELEASE.md)，不得替代上方 0.2.16 当前证据。
...
以上是当前候选源码合同，不是发布或人工通过声明。两份 v0.2.12 报告原文、现场数据库和截图集未取得；真实凭据管理器、原生保存对话框、外部文件打开、真人视觉和真实系统百分之二百缩放未执行。
...
> 源码复核：2026-08-29  
> 当前候选版本：`0.2.16`（工作区验证中；尚未形成唯一干净发布提交、标签或 Release）；现有 `v0.2.15` 及更早版本的标签、安装包和来源证据保持不变  
> 本轮边界：完整门禁、真实供应商、最终安装程序真机、真人交互和系统级 200% 缩放仍未完成

本文说明直接模式、专业模式、本地设定整理、StoryMemory 投影和长篇一致性调查在普通界面中的
当前交互。领域内部保留隔离结果、分流、过程记录和后台步骤等精确代码名称；普通用户一级导航
继续只有正文、规划、设定、检查。

以下来源指纹仅属于 v0.2.6 历史公开工程预发布证据，不是 0.2.16 的当前发布来源。该历史发布源由 annotated tag object `51dfd64ba22e9771131f251cdc778ee06f89192d` peel 到 commit
