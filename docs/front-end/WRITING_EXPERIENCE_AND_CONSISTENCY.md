...
以上是 0.2.15 当前工作区的界面合同，不是发布或人工通过声明。聚焦回归、准确根因、未执行项目和发布边界见 [0.2.15 修复与发布验证记录](../execution/2026-08-29-V0215-RELEASE.md)。完整桌面、数据、原生、正式构建、浏览器端到端、真实安装、真实服务、真人交互和 Windows 系统级 200% 缩放仍待主任务门禁；未知提交、安装包、校验值、标签和 Release 链接不会在事实产生前写入文档。
...
> 源码复核：2026-08-29  
> 当前候选版本：`0.2.15`（工作区验证中；尚未形成唯一干净发布提交、标签或 Release）；现有 `v0.2.14` 及更早版本的标签、安装包和来源证据保持不变  
...
以下来源指纹仅属于 v0.2.6 历史公开工程预发布证据，不是 0.2.15 的当前发布来源。该历史发布源由 annotated tag object `51dfd64ba22e9771131f251cdc778ee06f89192d` peel 到 commit
