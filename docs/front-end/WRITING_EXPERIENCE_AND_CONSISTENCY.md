...
- 当前工作树全量自动化门禁已经通过，但不等于真实服务、最终安装程序、系统百分之二百缩放或发布验收。

> 文档状态：`SUPPORTING_CURRENT`  
> 源码复核：2026-08-22  
> 当前实现：`0.2.7` 工作树完整源码门禁已通过；最新公开版本仍为 [`v0.2.6`](https://github.com/gugubugugu0826/InkShadow/releases/tag/v0.2.6)  
> 本轮边界：真实模型、候选端到端、系统百分之二百缩放、打包程序与发布尚未完成
...
这些实现后来已经随 v0.2.6 公开工程预发布冻结；它们不能作为当前 v0.2.7 工作树的全量、真实 Windows Tauri 或真实模型证据。当前状态见
