...
这些实现尚未完成最终全量、真实 Windows Tauri、真实模型对账或发布；当前状态见
...
直接模式中的选区替换、整章重写、删除/覆盖已有段落、跨章节移动、合并多个 Candidate 和重大
StoryFact 修改也走此类明确确认，不因模式简化而降低安全门槛。
...
专业/直接续写共用 `continuation-generation-disclosure.ts`：prepare 为 0 call，确认和 Provider 边界前重算完整
inspection/pricing/privacy/source/version fingerprint。只有直接模式可在同一精确 fingerprint 上复用持久 grant；
价格、路由、隐私、源版本或能力变化后都为 0 dispatch 并要求重新确认。
快速 AI 连接与设置页的固定能力探针同样先显示精确目标、1 次调用、0 次重试、最多 64 输出 token、费用未知，
