...
Data `0072`–`0075` / Tauri `75`–`78` 已随 `v0.2.7` 最终候选冻结，并重构直接模式、
方向用途、用户故事事实治理、不可变版本的本地整理责任和生成尝试隐私快照。最终候选、远端门禁、未签名打包和公开预发行已完成；真实供应商、最终安装程序真机与系统百分之二百缩放仍未执行；
...
下表键用于 `createDevelopmentRuntime(window.localStorage)`；正式 Tauri 业务数据使用 SQLite。批量整理设定的未完成批次通过 `runtime.authorRecovery` 访问：浏览器开发适配器使用等价持久存储，正式桌面端只使用 `author_recovery_records`，不会回退到 localStorage。

批量整理恢复以项目隔离，逐条修改按修订号串行保存。输入区不会在作者键入时隐式删除旧批次；只有“放弃这批内容并重新开始”或全部待确认设定安全落盘后才会条件删除恢复记录。坏记录和未来 schema 只显示内容无关的中文保护提示，原始内容保持不变。
...
## 2026-08-28 两轮缺陷统一修复事实

本轮界面事实、证据边界和 21 项缺陷矩阵统一记录在
[2026-08-28-V0214-UNIFIED-DEFECT-REMEDIATION.md](../execution/2026-08-28-V0214-UNIFIED-DEFECT-REMEDIATION.md)。

- 恢复整篇旧版本后，默认续写位置回到当前稳定正文末尾；作者随后主动移动光标时仍采用作者位置。
- 长续写与稳定正文存在完全或高度相似的长前缀时，隔离原文不改写，接受前必须展示本机证据并取得明确决定。
- 专业创建和批量设定按人物、身份、关系、地点、世界背景与写作约束分别进入待确认；未知类型保留原句。
- 部分输出在隔离建议和调用账本中使用同一事实口径，并显示已保留的可见字符数。
- 模型中心内部标签继续归属模型中心；设置快速跳转等待目标渲染后滚动并聚焦，重复点击同一入口仍会执行。
- 空项目名和章节名就地提示并回焦；历史长名称不截断数据库值，列表使用有界显示。
- 导入导出、高缩放布局、未完成创作卡片、设定错误和停用说明均使用可访问、可回焦的生产交互。

上述自动化事实不等于真人鼠标、真实触摸、真人视觉或 Windows 系统级 200% 缩放验证。
