...
已有连续状态的重大变化仍保持待确认，普通变化可撤销，所有历史证据继续绑定不可变版本。
...
| 路由                                                          | 页面文件                                                                                | 页面内容与主要操作                                                                                                                                                                               | 条件与数据来源                                                                                        |
| ------------------------------------------------------------- | --------------------------------------------------------------------------------------- | ------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------ | ----------------------------------------------------------------------------------------------------- |
| `/projects/:projectId/chapters/:chapterId`                    | `editor-page.tsx`                                                                       | 正文编辑、自动/手动保存、撤销重做、查找替换、排版、光标与滚动状态、版本历史、崩溃草稿、章节隐私、短/标准/长/自定义续写、动态预算、生成预检、上下文/Skill 参考、partial 恢复、候选质量提示和 Diff | 项目、章节、版本、恢复草稿、候选、Model Hub、生成治理、`runtime.novelSkills`、上下文 trace 和任务中心 |
...
手动保存成功会先按稳定版本 ID 登记可恢复的本地派生任务，只更新本地搜索和故事关联。章节摘要与
连续状态字段在新任务、旧任务恢复、人工重试和 worker 启动时都被强制关闭；自动保存同样只落盘。
这些保存路径精确产生 0 次 Provider 调用，派生失败也不撤销已经成功的正文和不可变版本。
...
`story.accepted-version.process` 后台任务尽力更新本地搜索索引和因果投影。当前该后台任务对所有
来源都只运行本地阶段：即使旧元数据曾要求章节摘要或故事状态，恢复时也会归一为关闭并保持
Provider 调用数为 0。任一派生步骤失败都不回滚或修改已接受正文。
...
- 当前本地迁移上限为 Data `0070` / Tauri `73`（70 个 Data + 3 个 story-core）：`0060`–`0065`
  保留 Novel Skill、付费评测、项目派发围栏与内容无关 Provider 发送边界；`0066`–`0068` 追加写作
  体验偏好、披露 grant、有界一致性调查四表和 active grant 上限修复；`0069` 为调查模型 step 预留
  content-free invocation UUID，并在账本 INSERT 时原子绑定 step/context trace；`0070` 只扩展可重建搜索投影的多粒度、父子定位与范围列。启动恢复会按是否
  越过 Provider 发送边界结清 ledger/run，并把仍非终态的 task 对账到终态，不自动重发。
  同一变更集继续复用 `0045` 派发 lease 与 Candidate/context output 原子版本围栏。旧页面说明中的
  较低数字只属于历史快照。当前恢复合同覆盖 172 张作者数据表，另有 1 张内容无关的原生项目派发
  租约表明确不恢复；`planned_invocation_id` 随调查 step 整表恢复。Tauri command 数量与 migration
  数量不能混用。
