...
`v0.2.6` 未发布修复工作树的页面增量：普通界面把实现术语改为“模型中心”“AI 建议草稿”
...
“使用这版”只等待正文、不可变版本和 AI 建议草稿决定的核心原子提交；页面随后立即读取已提交
状态，搜索、故事关联和设定整理等可重建派生转入后台，不再把派生等待伪装成仍在接受事务中。
...
- 当前工作树迁移上限为 Data `0071` / Tauri `74`（71 个 Data + 3 个 story-core）：`0060`–`0065`
  ...
  content-free invocation UUID，并在账本 INSERT 时原子绑定 step/context trace；`0070` 只扩展可重建搜索投影的多粒度、父子定位与范围列；`0071` 新增独立能力验证调用任务和能力证据外键。启动恢复会按是否
