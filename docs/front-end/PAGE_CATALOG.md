...
拒绝截断结果。首次成功且当前不存在任何任务路由记录时，智能推荐可建立 16 类文本任务分工；已有启用、禁用或自定义分工都不
覆盖，其他能力继续等待各自证据。诊断下载会列出最近脱敏 AI 失败和 request ID，但不含作品、
...
- 当前本地迁移上限为 Data `0056` / Tauri `59`；`0056` 只为现有 Model Hub 能力扫描和调用事实
  追加可空、脱敏的失败诊断字段与索引，旧页面说明中的较低数字只属于历史快照。
