...
> 2026-08-22 当前工作树增量：直接模式全局常驻入口只保留创作首页和作品库，项目内一级导航只保留正文、规划、设定、检查；命令搜索、任务、调用费用、模型状态和诊断入口只在专业模式显示。两种模式的生成结果都先隔离，每次远程发送逐次披露，明确“使用这版”后才写正文；一次点击远程授权和自动正文仍待独立授权。设定页的通用修改、历史恢复和合并只适用于纯文本事实，结构化/因果事实只开放安全治理动作。
