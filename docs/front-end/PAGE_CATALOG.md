...
| `/create/idea`         | `apps/desktop/src/pages/idea-journey-page.tsx`                                | 一句话开书、原位 AI 连接、三个固定开头建议槽、自己写/本地示例、一次一个问题及有限动态问题计划；显示 N/M、百分比、剩余重点和扩展原因，支持选项/自定义/跳过/返回/重生成/随时结束和中断恢复；创建前可编辑书名与故事摘要并查看来源和回答摘要；真实开头固定空白项目/章节/版本，建议只进隔离 Candidate，作者已显式启用实验写作方法时才附带精确 Skill/trace 回执 | `runtime.creativeJourneys`、`runtime.projectSeeds`、Model Hub、系统凭据库、`runtime.novelSkills`、候选与版本用例 |
...
开书提问不是“三问上限”。每次新旅程先建立 5 个重点的问题计划；作者的回答若暴露新的必要
边界，可一次扩展一个尚未问过的重点，整个计划最多 12 个唯一重点。页面始终显示当前第 N/M 问、
完成百分比、剩余重点及“为什么从原 M 扩展”的原因；作者可以返回上一问、跳过当前问，或随时
结束提问并进入创建摘要，因此不会在没有可见终点的情况下无限追问。回答和跳过状态只更新可恢复
旅程与 `ProjectSeed`，不会自动重写已选择的开头，也不会写入正式正文；计划完成后如作者明确选择，
最多再执行一次“根据全部回答重写”。

真实供应商开头固定保留三个彼此隔离的槽位，分别从即时行动、人物对话和悬念线索角度并发生成；
每槽独立显示状态、耗时、字数、重试、继续补全和保留动作。批次与请求 ID、旅程 revision CAS 会
阻止取消后或旧批次的迟到结果覆盖当前选择。`MODEL_OUTPUT_TRUNCATED` 只有在这个开书提案边界已
收到至少 160 个可见字符时才能成为显式 `partial` 草稿，并可继续补全或重新生成；普通正文、改写
和其他 Candidate 仍严格拒绝截断结果。
...
对应字段和解决办法。供应商预设可恢复默认地址与必要字段但不覆盖已保存密钥、连接、目录或路由；
...
- 当前本地迁移上限为 Data `0064` / Tauri `67`：`0060` 新增 Novel Skill registry/snapshot，`0061`
