...
| 路由        | 页面文件            | 页面内容与主要操作                                                                                                                                                                                                                                                                                                                                                                                                                                                              | 条件与数据来源                                                                                                                                                                                                                                                                          |
| ----------- | ------------------- | ------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- | --------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| `/settings` | `settings-page.tsx` | 外观；正文阅读与自动保存；数据与隐私；按项目忘掉全部 AI 记忆；Model Hub 普通/专家模式、供应商、权威 hydration 状态、目录刷新失败时保留缓存、官方可选目录、22 项任务候选与连接返回 intent、文本/严格结构/翻译能力探针、官方资料 fallback、时效性 Provider 推荐、能力证据、小说任务分工；图片/基础评测；专家模式下显式展开的写作方法 A/B 评测（本地准备、报价、单独商业授权、手动开始、盲评）；旧模型兼容；同步安全、数据库检查、备份恢复、安全更新、schema v3 脱敏诊断和导入导出 | `useAppearancePreference`、`editor-preferences-store`、`runtime.story.memoryService`、`modelHub`、`credentials`、`modelGateway`、`modelRouting`、`selectable-model-catalog-registry`、`model-hub-connection-intent`、`runtime.novelSkillPaidEvaluation`、`maintenance`、`secureUpdater` |
...
拒绝截断结果。首次成功且当前不存在任何任务路由记录时，智能推荐可建立 16 类文本任务分工；
...
- 当前本地迁移上限为 Data `0065` / Tauri `68`：`0060` 新增 Novel Skill registry/snapshot，`0061`
  新增并硬化 content-free 评测账本，`0062` 只补上持有既有项目派发 lease 时项目不得离开 active，
  `0063` 新增精确目标、商业授权、reservation 与盲评合同，`0064` 冻结内容无关的派发前权威，
  `0065` 只追加内容无关的 Provider 发送边界时间，用于重启时安全区分未发送和结果不明确；
  同一变更集的 Rust/TS 代码把 `0045` 已有 lease 覆盖到回环本地派发，并加强 Candidate/context output
  原子版本围栏。旧页面说明中的较低数字只属于历史快照。当前恢复合同覆盖 166 张作者数据表，另有 1 张
  内容无关的原生项目派发租约表明确不恢复。Tauri command 数量与 migration 数量不能混用。
