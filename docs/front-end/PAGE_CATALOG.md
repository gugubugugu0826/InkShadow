...
| `/create/idea`         | `apps/desktop/src/pages/idea-journey-page.tsx`                                | 一句话开书、原位 AI 连接、三个固定开头建议槽、自己写/本地示例、一次一个问题及有限动态问题计划；显示 N/M、百分比、剩余重点和扩展原因，支持选项/自定义/跳过/返回/重生成/随时结束和中断恢复；创建前可编辑书名与故事摘要并查看来源和回答摘要 | `runtime.creativeJourneys`、`runtime.projectSeeds`、Model Hub、系统凭据库、候选与版本用例 |
...
| `/projects/:projectId/story`      | `story-governance-page.tsx`                                                     | 统一事实与治理、人物/世界对象聚合、人物关系双端点列表、自然语言设定候选、严格 Story Settings JSON 导入/导出及旧开书设定整理；章节摘要、回填计划、故事变化、AI 参考历史、写作偏好、旧记忆/正式记录和人工记忆合并；旧自由 What-if 只读保留              | `runtime.story`、`runtime.storySettingsImport`、`runtime.contextTraces`；写入、冲突、记忆合并和回填都需要明确人工决定     |
...
| 路由        | 页面文件            | 页面内容与主要操作                                                                                                                                                                                                                              | 条件与数据来源                                                                                                                                                                  |
| ----------- | ------------------- | ----------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- | ------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| `/settings` | `settings-page.tsx` | 外观；正文阅读与自动保存；数据与隐私；按项目忘掉全部 AI 记忆；Model Hub 普通/专家模式、供应商、64-token 文本能力探针、能力证据、小说任务分工；图片/基础评测；旧模型兼容；同步安全、数据库检查、备份恢复、安全更新、schema v2 脱敏诊断和导入导出 | `useAppearancePreference`、`editor-preferences-store`、`runtime.story.memoryService`、`modelHub`、`credentials`、`modelGateway`、`modelRouting`、`maintenance`、`secureUpdater` |
...
Prompt、模型回答、推理正文、原始供应商错误或凭据。schema v2 把旧兼容指标命名为
...
- 当前本地迁移上限为 Data `0059` / Tauri `62`：`0057` 补齐 Model Hub 内容质量任务合同，`0058`
  保存 Story Settings 导入收据，`0059` 区分可估费用与价格未知；旧页面说明中的较低数字只属于
  历史快照。Tauri command 数量仍为 59，不能与 migration 数量混用。
